...
**Prisma apps:** container services (api, billing-api, couriers-api) use `@prisma/adapter-pg`; OpenNext Workers (console, widgets-api) use `@prisma/adapter-neon`, because workerd cannot open the raw TCP socket `pg` needs. Both take Neon pooled connection URLs, so **no Hyperdrive binding is needed** — the owning app's database URL is supplied as a Worker secret. Migrations run in GitHub Actions, never in the Worker or the Cloudflare build. See `docs/cloudflare.md`.
...
- Read `.claude/rules/production-render-errors.md` before debugging a page that fails only in a production build (React #441, Serwist `no-response`), before passing a prop from a Server Component to a Client Component, and before adding an authenticated route segment. It holds the logs-first debug procedure, the `pnpm check:rsc-boundaries` gate, and the segment `error.tsx` requirement.
