...
*(Note: `@876/work-ui` is currently not listed as a dependency of any host app; if added to an app in the future, `check:tailwind-sources` will immediately require its `@source` glob).*
...
```
*(Note on package naming: `packages/projects/package.json` defines package `@876/projects` which has no build script; `apps/projects/package.json` defines package `@876/projects-app`, which owns the Next.js build).*
