# Brief — port services/auth_telemetry.py to Express/TypeScript

**Working directory:** `/workspaces/876/apps/api`

## The goal

Port `services/auth_telemetry.py` (350 lines) to
`src/services/auth-telemetry.ts`, behaviour-identical, plus a Vitest suite
at `src/services/__tests__/auth-telemetry.test.ts`.

Read the Python source in full first, including every repository and model
it touches (`apps/api/db/repositories/`, `apps/api/db/models/`) and the
matching Prisma models in `apps/api/prisma/schema/`.

## Files you may create

- `src/services/auth-telemetry.ts`
- `src/services/auth-telemetry.repository.ts` (only if it needs the database)
- `src/services/__tests__/auth-telemetry.test.ts`

## Files you must NOT touch

Anything else. Other agents are concurrently writing every other
`src/services/*` file. Do not edit `src/http/**`, `src/modules/**`,
`prisma/**`, or any `.py` file. Do not run `git commit`.
