...
## 3. Binding repository and mobile correctness rules
...
For the Projects Mobile run, these two run-specific files are also **mandatory before any implementation work**:

- `plans/sep/16-projects-mobile/00-local-environment-preflight.md`
- `plans/sep/16-projects-mobile/01-react-native-expo-docs-and-ai-guardrails.md`

`01-react-native-expo-docs-and-ai-guardrails.md` is a phase gate, not optional reference material. Muse must re-check the relevant **current official Expo, React Native, and library documentation before every native/mobile implementation phase**, record the docs consulted and current selected versions in that phase report, and stop rather than invent an API when current docs cannot be verified.
...
Only official/current documentation should be used when implementing Expo-native setup. The facts in this section are a dated baseline, **not a substitute for the mandatory fresh-doc check before each implementation phase**.
...
### Stable-vs-latest React Native trap

As of 2026-09-16, React Native `0.87` is a newer standalone stable React Native release, but stable Expo SDK 57 still targets React Native `0.86`. Expo SDK 58 is currently beta and includes a React Native `0.88` release candidate.
...
Therefore Muse must never equate "latest React Native" with "correct React Native for this Expo app." The app follows the React Native version supported by the selected Expo SDK. Do not adopt an Expo beta/canary merely because it is newer unless the user explicitly approves that product risk.
...
Expo describes development builds with `expo-dev-client` as the appropriate workflow for production applications that need native modules/configuration.
...
Use a development build from the start. Do not make Expo Go compatibility a product constraint. Adding native-code packages or changing relevant native app configuration may require rebuilding the development client; a JS reload cannot add native code to an existing APK.
...
The native app is a **public OAuth client**: PKCE is required and no OAuth client secret belongs in the bundle. Secure token material belongs in platform-backed secure storage, not AsyncStorage.

### New Architecture

Expo's current guidance says the New Architecture is always enabled in SDK 55 and later.

Source: <https://docs.expo.dev/guides/new-architecture/>

Do not add `newArchEnabled: false` or choose stale native packages that require disabling the New Architecture.

### Safe areas / Android edge-to-edge

React Native's built-in `SafeAreaView` is deprecated. Use the current Expo-supported `react-native-safe-area-context` approach and test Android edge-to-edge behavior on-device.
...
Projects, Issues, Comments, Notifications, and member pickers can become unbounded. Use React Native virtualized lists instead of rendering large server collections through a mapped `ScrollView`.
...
Do not defeat virtualization with unnecessary same-direction `ScrollView` nesting. Keep list rows light and use durable item IDs as keys.
...
### Mandatory anti-slop source

The complete mobile-specific anti-pattern checklist and phase-by-phase official docs map are maintained in:

```text
plans/sep/16-projects-mobile/01-react-native-expo-docs-and-ai-guardrails.md
```

That file is binding throughout implementation and must be re-read whenever a phase changes navigation, auth, native dependencies/config, storage, lists, forms, images, Query lifecycle, or EAS behavior.
...
### AD-13 — Current docs verification is a release gate

For every phase that changes native/mobile code, the phase report must include a `Current docs verification` section containing:

```text
checked date
selected Expo SDK
React Native version for that Expo SDK
relevant installed package versions
official documentation URLs consulted
deprecated/stale patterns explicitly avoided
any conflict between docs and repository assumptions
```

A native/mobile phase is incomplete without that evidence.
...
**Goal:** prove current repo/API assumptions and current mobile-tooling guidance before changing code.

Tasks:

- [ ] Re-read all binding rules on the implementation branch.
- [ ] Run the environment verification from `00-local-environment-preflight.md` and record what is already installed vs missing.
- [ ] Read `01-react-native-expo-docs-and-ai-guardrails.md` fully.
- [ ] Re-fetch current official Expo SDK, Expo changelog, Expo Router, monorepo, development-build, New Architecture, and React Native release documentation.
- [ ] Record current stable Expo SDK and distinguish it from any beta/canary release.
- [ ] Record the React Native and React versions supported by the selected stable Expo SDK; do not independently choose the newest React Native version.
...
- [ ] Run `pnpm create expo-app` help/template inspection and confirm the current stable template before accepting generated versions.
- [ ] Record exact production Core/OAuth public origin.
- [ ] Record any premise that is false before proceeding.

**No implementation should proceed from an unverified premise or stale mobile API memory.**
...
Before code, re-check the current official AuthSession, WebBrowser, SecureStore, and Expo Router authentication docs and record them in the phase report.
...
Before scaffold/dependency changes, repeat the current Expo SDK/monorepo/New Architecture/development-build documentation check. Use Expo-compatible installation commands rather than guessed `@latest` native dependencies.

Tasks:

- [ ] Create `apps/projects-mobile` using the current selected stable Expo template through pnpm/Expo tooling.
- [ ] Use Expo Router.
- [ ] Install `expo-dev-client` with Expo-compatible package resolution.
...
- [ ] Do not add `newArchEnabled: false`.
- [ ] Run `expo-doctor` and diagnose any root React vs Expo-supported React resolution issue before changing shared dependency versions.
- [ ] Review changed files against the anti-slop search patterns in `01-react-native-expo-docs-and-ai-guardrails.md`.
...
- [ ] Implement AuthSession/WebBrowser PKCE flow using the current documented APIs.
...
- [ ] Protect authenticated route group using current Expo Router protected-route APIs.
- [ ] Treat protected routes as UX only; API authorization remains authoritative.
...
Before code, check the current TanStack Query React Native guide matching the installed major version.
...
- [ ] Do not duplicate server query results into component state as a second cache.

### Phase 7 — Projects and Issues

**Goal:** all authorized work is discoverable and actionable.

Before list work, re-check current React Native `FlatList`/virtualization guidance and Expo safe-area guidance.

Tasks:

- [ ] Projects list/pagination/search using virtualized lists for unbounded collections.
- [ ] Project detail.
- [ ] Organization-wide Issues list using server pagination/filters.
...
- [ ] Do not render unbounded Issue/Project data by mapping inside a `ScrollView`.
- [ ] Use current safe-area APIs rather than deprecated core `SafeAreaView`.

### Phase 8 — Comments

**Goal:** complete mobile Issue discussion workflow.

Before code, re-check current `TextInput`, keyboard avoidance, list virtualization, and accessibility docs.

Tasks:

- [ ] Load all comments for Issue.
- [ ] Native comment composer with keyboard-safe behavior tested on Android.
- [ ] Create/edit/delete.
...
- [ ] Use virtualized lists for unbounded feeds.
...
- [ ] Re-run the complete mobile anti-slop diff review from `01-react-native-expo-docs-and-ai-guardrails.md`.
...
- [ ] Validate safe areas, Android edge-to-edge, keyboard behavior, lists, and accessibility-critical interactions on device.
...
Because Muse may produce confident but stale React Native/Expo patterns, **model memory is never the authority for native APIs**. Before each mobile phase it must follow `01-react-native-expo-docs-and-ai-guardrails.md`, fetch current official docs, and include current-doc evidence in its report.
...
For each native/mobile phase, verification additionally requires:

- a `Current docs verification` section in the phase report;
- anti-slop diff review using the search checklist from `01-react-native-expo-docs-and-ai-guardrails.md`;
- a development-client rebuild when native dependency/config changes require one;
- physical-device verification for device/native-specific behavior.
...
- [ ] all unbounded lists paginate and use appropriate native virtualization;
...
- [ ] no hand-maintained Metro monorepo config exists without a documented measured reason;
- [ ] no deprecated core `SafeAreaView` is introduced;
- [ ] no legacy-architecture disable switch is introduced;
...
- [ ] SDK/package version changes are made using current Expo-compatible tooling rather than guessed semver;
- [ ] every native phase cites current official docs rather than relying on model memory.
...
### R6 — Non-frontier local model generates stale mobile patterns

Muse may produce plausible React web or historical React Native patterns that compile but are deprecated, inefficient, insecure, or wrong for the selected Expo SDK.

Mitigation: mandatory current official docs lookup before every mobile phase, binding `01-react-native-expo-docs-and-ai-guardrails.md`, required report evidence, anti-slop diff searches, Expo Doctor, and physical-device verification.

### R7 — "Latest" package mismatch across Expo and standalone React Native

A newer standalone React Native version may exist than the version supported by the current stable Expo SDK; a newer Expo SDK may still be beta.

Mitigation: choose the Expo SDK release policy first, then use the React/React Native versions supported by that SDK. Beta/canary adoption requires explicit user approval.
...
Every future native/mobile execution report must include the current-doc verification block required by `01-react-native-expo-docs-and-ai-guardrails.md`.
...
- [x] Create local machine/environment preflight.
- [x] Create React Native / Expo current-doc and AI-slop guardrails.
- [x] Make the current-doc guardrail mandatory in the master plan/read order.
...
- local environment preflight exists;
- React Native / Expo current-doc and AI-slop guardrails exist and are mandatory;
...
- implementation should begin with **environment preflight + Phase 0 reconnaissance only after explicit user authorization**.

The local driver should follow the required read order in `README.md`: environment preflight, React Native / Expo docs-and-guardrails, this master plan, then the orchestration/local administration files. It must re-read the binding repository rules from the current branch and verify that the integration branch has not drifted from this handoff before beginning.
