# Brief — convert the Billing subscription-catalog sections to the list/detail split view
...
| #   | Section directory (under `apps/billing/src/app/(app)/(subscription-management)/(catalog)/`) | Record segment |
| --- | ------------------------------------------------------------------------------------------- | -------------- |
...
**None of these five has a `layout.tsx` yet — create one for each.** They all
sit under a shared `(catalog)/layout.tsx`; do not modify that shared file.
...
- **Do not** edit anything outside the five section directories listed above.
  In particular do not touch `packages/`, `apps/invoice`, `apps/crm`,
  `apps/console`, `detail-layout.tsx`, `(catalog)/layout.tsx`,
  `(subscription-management)/layout.tsx`, any other Billing section, or any
  `[id]/` directory. Another agent is working in
  `apps/billing/src/app/(app)/{customers,items,(sales)}` **right now** — going
  outside your five directories will collide with it.
...
Write `/root/projects/876/.claude/reports/agy/2026-09-02-billing-catalog-list-detail-split.md`
