...
export function buildSenderRoutes() {
  ...
  // Declared before '/:senderId' so Express cannot match "managed" as a sender id.
  router.post(
    '/managed',
    async (req: Request<OrganizationScopedParams>, res) => {
      const organizationId = req.params.organizationId
      if (!organizationId)
        return sendError(res, 'communications/invalid-request')

      const parsed = ensureManagedSenderSchema.safeParse(req.body)
      if (!parsed.success)
        return sendError(res, 'communications/invalid-request')

      return sendResult(
        res,
        await ensureManagedSender(organizationId, parsed.data)
      )
    }
  )
  ...
  router.get(
    '/:senderId',
    async (
      req: Request<OrganizationScopedParams & { senderId: string }>,
      res
    ) => {
