...
export async function handleGetWorkTasks(request: Request, invoiceId?: string) {
  ...
  const result = await work.tasks.list(auth.orgId, {
    ...(context?.context
      ? { context: toWorkContext(context.context) }
...
export async function handlePostWorkTask(request: Request, invoiceId?: string) {
  ...
  const candidate = {
    ...
    ...(context?.context
      ? {
          context: toWorkContext(context.context),
          primaryLink: toCreateWorkTaskLinkInput(context.context),
        }
      : {}),
    ...(parsed.data.listId ? { listId: parsed.data.listId } : {}),
