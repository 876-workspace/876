...
function membership(overrides: Record<string, unknown> = {}) {
  return {
    ...
    entitled: true,
    revoked_at: null,
    effective_permissions: ['dashboard.view', 'invoices.view'],
    ...overrides,
  }
}
...
beforeEach(() => {
  vi.clearAllMocks()
  mocks.resolvePlatformAppId.mockResolvedValue('rap_invoice')
  mocks.getFeatures.mockResolvedValue({ featureKeys: [] })
})

describe('Invoice access context', () => {
  it('uses effective permissions unchanged', async () => {
    ...
    await expect(
      ...
      status: 'ok',
      context: { permissions: ['dashboard.view', 'invoices.view'] },
    })
  })
  ...
  it('uses enabled feature keys', async () => {
    mocks.retrieve.mockResolvedValue({ data: membership(), error: null })
    mocks.getFeatures.mockResolvedValue({ featureKeys: ['invoice-reports'] })
    await expect(
      resolveAccessContext('user_8', 'org_8')
    ).resolves.toMatchObject({
      status: 'ok',
      context: { features: ['invoice-reports'] },
    })
  })
  ...
  it('answers permission and feature helpers without fetching', async () => {
    ...
    expect(canAccess(result.context, 'dashboard.view')).toBe(true)
    expect(hasAccessFeature(result.context, 'invoice-reports')).toBe(false)
  })
})
