...
describe('resolveReportPageParams', () => {
  ...
  it('ignores oversized or inverted explicit ranges', () => {
    expect(
      resolveReportPageParams(
    ...
    const inverted = resolveReportPageParams(
      { preset: 'this-week', from: String(MONTH_TO), to: String(MONTH_FROM) },
      TIME_ZONE,
      NOW
    )
    expect(inverted.from).toBeLessThan(inverted.to)
  })

  it('keeps a late-night Jamaica sale inside its calendar month', () => {
    const late = Date.UTC(2026, 9, 1, 4, 30, 0) / 1000
    const resolved = resolveReportPageParams({ preset: 'this-month' }, TIME_ZONE, NOW)
    expect(late >= resolved.from && late < resolved.to).toBe(true)
  })
})
