...
export function QuoteEditForm({ quoteId, initial }: { quoteId: string; initial: DocumentUpdateParams }) {
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await client.documents.updateQuote(quoteId, {
        issueAt: issue,
        expiresAt: expires,
  ...
  return (
    <form onSubmit={submit} className="max-w-3xl space-y-6">
      <section className="876-card space-y-5 p-5 sm:p-6">
        <FormRow label="Quote date"><Input type="date" value={issueAt} onChange={(event) => setIssueAt(event.target.value)} disabled={isPending} /></FormRow>
        <FormRow label="Expires"><Input type="date" value={expiresAt} onChange={(event) => setExpiresAt(event.target.value)} disabled={isPending} /></FormRow>
        <FormRow label="Customer note"><Textarea value={notes} onChange={(event) => setNotes(event.target.value)} disabled={isPending} /></FormRow>
        <FormRow label="Terms and conditions"><Textarea value={terms} onChange={(event) => setTerms(event.target.value)} disabled={isPending} /></FormRow>
      </section>
