...
export default async function EditRecurringInvoicePage({ params }: Props) {
  ...
  const [profile, customers, currencies, items, variants] = await Promise.all([
  ...
  const initial: RecurringInvoiceFormInitial = {
    ...
    currency: String(record.currency ?? 'JMD'),
    ...
    lines: (Array.isArray(record.lines) ? record.lines : []).map((entry) => {
      ...
      return {
        ...
        unitAmount: typeof line.unitAmount === 'string' ? line.unitAmount : null,
  ...
  return (
    <Page>
      ...
      <InvoiceRecurringInvoiceEditForm
