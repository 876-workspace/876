...
import { formatRecurringFrequency } from '@876/billing-ui/recurring-invoices-list'
import {
...
export default async function RecurringInvoiceDetailPage({ params }: Props) {
  ...
  const customerName =
    !customer || customer.error
      ? '—'
      : text(
          (customer.data as unknown as Record<string, unknown>).name,
          '—'
        )
  ...
  return (
    <DetailCard
      aria-label={`Recurring invoice details: ${text(profile.profileName)}`}
    >
      <DetailCardHeader
        icon={
          <DetailCardIcon>
        ...
        actions={
          <InvoiceRecurringInvoiceLifecycleActions
      ...
      <DetailCardBody className="space-y-8">
        <DetailCardHeadline
          value={formatMoney(String(profile.totalAmount ?? '0'), currency)}
        ...
        <DetailCardSection title="Schedule">
          <DetailCardFacts>
            ...
            />
            <DetailCardFact label="Generated" value={String(generatedCount)} />
            <DetailCardFact
