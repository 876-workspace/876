...
describe('RecurringInvoicesListData', () => {
  ...
  it('threads a paused status into the SDK list call', async () => {
    // ARRANGE
    navigationTestState.searchParams = new URLSearchParams('status=paused')
  ...
  it('lists unfiltered when the status is all or missing', async () => {
    ...
    // ASSERT
    await waitFor(() => expect(mocks.listProfiles).toHaveBeenCalledWith({}))
  ...
  it('renders the fetched profiles with resolved customer names', async () => {
    ...
    // ASSERT
    expect(await screen.findByText('Monthly retainer')).toBeTruthy()
    expect(await screen.findByText('Alejandra Reyes')).toBeTruthy()
  ...
  it('renders an inline error instead of rows when the list fails', async () => {
    ...
    // ASSERT
    expect(await screen.findByText('Billing is down.')).toBeTruthy()
