...
export default async function RolesLayout({ children }: { children: ReactNode }) {
  ...
  if (context.status === 'unavailable')
    return <AppError title="Roles could not be loaded" error={{ code: 'invoice/access-unavailable', message: 'Access could not be verified. Try again.' }} variant="section" />

  const access = await resolveInvoiceFinanceAccess(context.context.orgId, context.context.userId, context.context.role)
  if (access.status === 'unavailable')
    return <AppError title="Roles could not be loaded" error={{ code: access.code, message: 'Finance access could not be verified. Try again.' }} variant="section" />
  if (!access.viewer.permissions.includes(ROLES_READ_PERMISSION)) redirect('/no-access')

  return (
    <InvoiceRolesShell
      canCreate={access.viewer.permissions.includes('roles:write')}
      list={<Suspense fallback={<div className="flex h-full min-h-0 flex-col" />}><RolesListData organizationId={context.context.orgId} /></Suspense>}
    >
      {children}
    </InvoiceRolesShell>
  )
