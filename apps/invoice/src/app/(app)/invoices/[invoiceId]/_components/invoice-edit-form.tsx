...
export function InvoiceEditForm({ invoiceId, status, initial }: Props) {
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if ((dueAt && due === null) || (!restricted && issueAt && issue === null)) {
      setError('Enter valid invoice dates.')
      return
    }
    setError(null)
    startTransition(async () => {
      const update: DocumentUpdateParams = {
        dueAt: due,
      ...
      const result = await client.documents.update(invoiceId, update)
      if (result.error) {
