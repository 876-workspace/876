...
const mocks = vi.hoisted(() => ({ permission: vi.fn(), tasks: vi.fn(), events: vi.fn() }))

vi.mock('@/lib/auth/api-permission', () => ({ requireApiPermission: mocks.permission }))
...
describe('Invoice request task and activity routes', () => {
  beforeEach(() => { vi.resetAllMocks(); mocks.permission.mockResolvedValue(access) })
