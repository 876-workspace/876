...
describe('Work recurrence-rules service', () => {
  it('create DAILY rule produces FREQ=DAILY', async () => {
    vi.mocked(repository.create).mockResolvedValue(
      row({ frequency: 'DAILY', rrule: 'FREQ=DAILY' }) as never
    )
    ...
    expect(result).toEqual(
      expect.objectContaining({ frequency: 'DAILY', rrule: 'FREQ=DAILY' })
    )
  ...
  it('create WEEKLY rule produces FREQ=WEEKLY', async () => {
    vi.mocked(repository.create).mockResolvedValue(
      row({ frequency: 'WEEKLY', rrule: 'FREQ=WEEKLY' }) as never
    )
    const result = await service.create('org_kingston_1', {
      frequency: 'WEEKLY',
      timeZone: 'UTC',
  ...
  it('create MONTHLY rule produces FREQ=MONTHLY', async () => {
    vi.mocked(repository.create).mockResolvedValue(
      row({ frequency: 'MONTHLY', rrule: 'FREQ=MONTHLY' }) as never
    )
    const result = await service.create('org_kingston_1', {
      frequency: 'MONTHLY',
      timeZone: 'UTC',
  ...
  it('create YEARLY rule produces FREQ=YEARLY', async () => {
    vi.mocked(repository.create).mockResolvedValue(
      row({ frequency: 'YEARLY', rrule: 'FREQ=YEARLY' }) as never
    )
    const result = await service.create('org_kingston_1', {
      frequency: 'YEARLY',
      timeZone: 'UTC',
  ...
  it('create with interval 2 includes INTERVAL=2', async () => {
    vi.mocked(repository.create).mockResolvedValue(
      row({ interval: 2, rrule: 'FREQ=WEEKLY;INTERVAL=2' }) as never
    )
    const result = await service.create('org_kingston_1', {
      frequency: 'WEEKLY',
      interval: 2,
    ...
    expect(repository.create).toHaveBeenCalledWith(
      expect.objectContaining({ rrule: 'FREQ=WEEKLY;INTERVAL=2' })
    )
  })

  it('create with interval 1 omits INTERVAL', async () => {
    vi.mocked(repository.create).mockResolvedValue(
      row({ interval: 1, rrule: 'FREQ=DAILY' }) as never
    )
    const result = await service.create('org_kingston_1', {
      frequency: 'DAILY',
      interval: 1,
  ...
  it('create with byDay includes BYDAY', async () => {
    vi.mocked(repository.create).mockResolvedValue(
      row({ byDay: ['MO', 'WE'], rrule: 'FREQ=WEEKLY;BYDAY=MO,WE' }) as never
    )
    const result = await service.create('org_kingston_1', {
      frequency: 'WEEKLY',
      byDay: ['MO', 'WE'],
  ...
  it('create with byMonthDay 31 is valid', async () => {
    vi.mocked(repository.create).mockResolvedValue(
      row({ byMonthDay: [31], rrule: 'FREQ=MONTHLY;BYMONTHDAY=31' }) as never
    )
    const result = await service.create('org_kingston_1', {
      frequency: 'MONTHLY',
      byMonthDay: [31],
  ...
  it('create with byMonth includes BYMONTH', async () => {
    vi.mocked(repository.create).mockResolvedValue(
      row({ byMonth: [1, 12], rrule: 'FREQ=YEARLY;BYMONTH=1,12' }) as never
    )
    const result = await service.create('org_kingston_1', {
      frequency: 'YEARLY',
      byMonth: [1, 12],
  ...
  it('create with count includes COUNT', async () => {
    vi.mocked(repository.create).mockResolvedValue(
      row({ count: 5, rrule: 'FREQ=DAILY;COUNT=5' }) as never
    )
    const result = await service.create('org_kingston_1', {
      frequency: 'DAILY',
      count: 5,
  ...
  it('create with untilAt includes UNTIL', async () => {
    const until = Math.floor(
      new Date('2026-12-31T00:00:00.000Z').getTime() / 1000
    )
    vi.mocked(repository.create).mockResolvedValue(
      row({
        untilAt: new Date(until * 1000),
  ...
  it('create with weekStart includes WKST', async () => {
    vi.mocked(repository.create).mockResolvedValue(
      row({ weekStart: 'MO', rrule: 'FREQ=WEEKLY;WKST=MO' }) as never
    )
    const result = await service.create('org_kingston_1', {
      frequency: 'WEEKLY',
      weekStart: 'MO',
  ...
  it('update changes frequency and rebuilds rrule', async () => {
    ...
    const result = await service.update('org_kingston_1', 'rrule_1', {
      frequency: 'WEEKLY',
    })
    expect(result).toEqual(expect.objectContaining({ frequency: 'WEEKLY' }))
    expect(repository.update).toHaveBeenCalledWith(
      'rrule_1',
      expect.objectContaining({ frequency: 'WEEKLY', rrule: 'FREQ=WEEKLY' })
    )
  })
  ...
  it('update omitting a field keeps the persisted value when rebuilding the RRULE', async () => {
    ...
    vi.mocked(repository.update).mockResolvedValue(
      row({ rrule: 'FREQ=WEEKLY;INTERVAL=2;BYDAY=FR;COUNT=8' }) as never
    )
    await service.update('org_kingston_1', 'rrule_1', { interval: 2 })
    expect(repository.update).toHaveBeenCalledWith(
      'rrule_1',
      expect.objectContaining({
        rrule: 'FREQ=WEEKLY;INTERVAL=2;BYDAY=FR;COUNT=8',
      })
    )
  })
  ...
  it('update clears count when null is supplied explicitly', async () => {
    ...
    vi.mocked(repository.update).mockResolvedValue(
      row({ count: null, rrule: 'FREQ=DAILY' }) as never
    )
