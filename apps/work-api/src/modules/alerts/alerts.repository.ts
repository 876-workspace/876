...
export const retrieve = (tenantId: string, alertId: string) =>
  prisma.workAlert.findFirst({ where: { tenantId, id: alertId } })
export const create = (params: CreateParams) =>
  prisma.workAlert.create({
    data: { id: `alert_${randomUUID().replaceAll('-', '')}`, ...params },
  })
export const update = (alertId: string, params: UpdateParams) =>
  prisma.workAlert.update({ where: { id: alertId }, data: params })
...
export const absoluteDue = (now: Date, limit = 500) =>
  prisma.workAlert.findMany({
    where: {
      status: 'SCHEDULED',
...
export const relativeCandidates = (limit = 500) =>
  prisma.workAlert.findMany({
