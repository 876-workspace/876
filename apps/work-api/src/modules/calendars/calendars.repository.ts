...
type Filter = { userId?: string; visibility?: string; limit: number; startingAfter?: string; endingBefore?: string }
...
export const retrieve = (tenantId: string, calendarId: string) => prisma.workCalendar.findFirst({ where: { tenantId, id: calendarId, deletedAt: null } })
export const primaryForUser = (tenantId: string, userId: string) => prisma.workCalendar.findFirst({ where: { tenantId, ownerUserId: userId, isPrimary: true, deletedAt: null } })
export async function create(params: CreateParams) {
  return prisma.$transaction(async (tx) => {
    const id = `calendar_${randomUUID().replaceAll('-', '')}`
    const uid = `${id}@work.876`
    const isPrimary = params.ownerUserId
      ? !(await tx.workCalendar.findFirst({ where: { tenantId: params.tenantId, ownerUserId: params.ownerUserId, isPrimary: true, deletedAt: null } }))
      : false
    const calendar = await tx.workCalendar.create({ data: { id, uid, ...params, isPrimary } })
...
export const update = (calendarId: string, params: UpdateParams) => prisma.workCalendar.update({ where: { id: calendarId }, data: params })
export async function remove(calendarId: string, deletedBy: string) {
  if (process.env.DELETION_MODE === 'hard') await prisma.workCalendar.delete({ where: { id: calendarId } })
  else await prisma.workCalendar.update({ where: { id: calendarId }, data: { deletedAt: new Date(), deletedBy } })
