...
export async function retrieve(organizationId: string, calendarId: string) {
  const tenant = await requireTenant(organizationId)
  if (isError(tenant)) return tenant
  const row = await repository.retrieve(tenant.id, calendarId)
...
export async function ensurePrimary(
  ...
  const tenant = await requireTenant(organizationId)
  if (isError(tenant)) return tenant
  const existing = await repository.primaryForUser(tenant.id, userId)
  if (existing) return serialize(existing, organizationId)
...
export async function update(
  ...
  const tenant = await requireTenant(organizationId)
  if (isError(tenant)) return tenant
  const current = await repository.retrieve(tenant.id, calendarId)
  if (!current) return null
  const row = await repository.update(calendarId, {
...
export async function remove(
  ...
  if (current.isPrimary)
    return getError('work/calendar-primary-delete-forbidden')
  return repository.remove(calendarId, deletedBy)
}
