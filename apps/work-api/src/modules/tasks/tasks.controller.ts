...
export async function listTasks(req: Request, res: Response) {
  ...
  const context = query.context_service
    ? { service: query.context_service, resource: query.context_resource!, id: query.context_id! }
    : undefined
  const result = await service.list(organizationId, {
    ...(context ? { context } : {}),
    ...(query.list_id ? { listId: query.list_id } : {}),
    ...(query.parent_task_id !== undefined ? { parentTaskId: query.parent_task_id || null } : {}),
...
export async function createTask(req: Request, res: Response) {
  const { organizationId } = organizationParamsSchema.parse(req.params)
  return sendWorkResult(res, await service.create(organizationId, createTaskBodySchema.parse(req.body)), 201)
}
export async function updateTask(req: Request, res: Response) {
  const { organizationId, taskId } = taskParamsSchema.parse(req.params)
  const result = await service.update(organizationId, taskId, updateTaskBodySchema.parse(req.body))
  if (!result) return sendWorkError(res, 'work/task-not-found')
  return sendWorkResult(res, result)
}
