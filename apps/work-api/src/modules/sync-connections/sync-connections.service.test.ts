...
describe('Work sync-connections service', () => {
  ...
  it('create credentialRef is a reference never a bearer token - raw token not persisted as credentialRef', async () => {
    ...
    vi.mocked(repository.create).mockResolvedValue(
      row({ credentialRef: 'cred_ref_safe' }) as never
    )
    await service.create('org_kingston_1', {
      userId: 'user_1',
      provider: 'GOOGLE',
      credentialRef: 'cred_ref_safe',
    })
    const stored = vi.mocked(repository.create).mock.calls[0]![0] as Record<
      string,
      unknown
    >
  ...
  it('create with caldavUrl persists it', async () => {
    vi.mocked(repository.create).mockResolvedValue(
      row({ caldavUrl: 'https://caldav.example.test/mandeville' }) as never
    )
    await service.create('org_kingston_1', {
      userId: 'user_1',
      provider: 'CALDAV',
      credentialRef: 'cred_1',
      caldavUrl: 'https://caldav.example.test/mandeville',
    })
    expect(repository.create).toHaveBeenCalledWith(
      expect.objectContaining({
        caldavUrl: 'https://caldav.example.test/mandeville',
  ...
  it('retrieve returns connection when found', async () => {
    vi.mocked(repository.retrieve).mockResolvedValue(
      row({ id: 'conn_found' }) as never
    )
    const result = await service.retrieve('org_kingston_1', 'conn_found')
    expect(result).toEqual(
      expect.objectContaining({ id: 'conn_found', object: 'sync_connection' })
    )
  })
  ...
  it('update transitions status ACTIVE->PAUSED', async () => {
    vi.mocked(repository.retrieve).mockResolvedValue(
      row({ status: 'ACTIVE' }) as never
    )
    vi.mocked(repository.update).mockResolvedValue(
      row({ status: 'PAUSED' }) as never
    )
  ...
  it('update stores syncCursor', async () => {
    ...
    vi.mocked(repository.update).mockResolvedValue(
      row({ syncCursor: 'cursor_abc' }) as never
    )
    await service.update('org_kingston_1', 'sync_conn_1', {
      syncCursor: 'cursor_abc',
    })
    expect(repository.update).toHaveBeenCalledWith(
      'sync_conn_1',
      expect.objectContaining({ syncCursor: 'cursor_abc' })
    )
  })
  ...
  it('update stores lastErrorCode', async () => {
    vi.mocked(repository.retrieve).mockResolvedValue(row() as never)
    vi.mocked(repository.update).mockResolvedValue(
      row({ lastErrorCode: 'auth_failed' }) as never
    )
    await service.update('org_kingston_1', 'sync_conn_1', {
    ...
    expect(repository.update).toHaveBeenCalledWith(
      'sync_conn_1',
      expect.objectContaining({ lastErrorCode: 'auth_failed' })
    )
  })
  ...
  it('update transitions status ACTIVE to REVOKED and clears the sync cursor', async () => {
    vi.mocked(repository.retrieve).mockResolvedValue(
      row({ syncCursor: 'cursor_old' }) as never
    )
    vi.mocked(repository.update).mockResolvedValue(
      row({ status: 'REVOKED', syncCursor: null }) as never
    )
    const result = await service.update('org_kingston_1', 'sync_conn_1', {
      status: 'REVOKED',
      syncCursor: null,
    })
    expect(repository.update).toHaveBeenCalledWith('sync_conn_1', {
      status: 'REVOKED',
      syncCursor: null,
    })
    expect(result).toEqual(
      expect.objectContaining({ status: 'REVOKED', syncCursor: null })
    )
  })

  it('update clears the credential reference when null is supplied', async () => {
    vi.mocked(repository.retrieve).mockResolvedValue(
      row({ credentialRef: 'vault_ref_mandeville_1' }) as never
    )
    vi.mocked(repository.update).mockResolvedValue(
      row({ credentialRef: null }) as never
    )
    await service.update('org_kingston_1', 'sync_conn_1', {
      credentialRef: null,
    })
