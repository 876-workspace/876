...
describe('Work my-work service', () => {
  it('aggregates tasks assigned to user within window', async () => {
    ...
    const result = (await service.retrieve(ORG, USER, FROM, TO)) as {
      tasks: { id: string }[]
    }
  ...
  it('task exactly on FROM edge is included, exactly on TO excluded', async () => {
    vi.mocked(tasks.list).mockResolvedValue({
      data: [
        taskRow({ id: 'task_from', dueAt: FROM, startAt: FROM }),
        taskRow({ id: 'task_to', dueAt: TO, startAt: TO }),
      ],
      hasMore: false,
    } as never)
    const result = (await service.retrieve(ORG, USER, FROM, TO)) as {
  ...
  it('aggregates events across visible calendars without duplication - sorted by start', async () => {
    vi.mocked(calendars.list).mockResolvedValue({
      data: [{ id: 'cal_a' }, { id: 'cal_b' }],
      hasMore: false,
    } as never)
    vi.mocked(events.list)
      .mockResolvedValueOnce({
        data: [eventRow({ id: 'event_b', startAt: FROM + 2000 })],
        hasMore: false,
      } as never)
      .mockResolvedValueOnce({
        data: [eventRow({ id: 'event_a', startAt: FROM + 1000 })],
        hasMore: false,
      } as never)
    const result = (await service.retrieve(ORG, USER, FROM, TO)) as {
      events: { id: string }[]
  ...
  it('scopes tasks to acting user only', async () => {
    ...
    expect(tasks.list).toHaveBeenCalledWith(
      ORG,
      expect.objectContaining({ assigneeId: USER })
    )
  })
  ...
  it('scopes calendars to acting user only', async () => {
    await service.retrieve(ORG, USER, FROM, TO)
    expect(calendars.list).toHaveBeenCalledWith(
      ORG,
      expect.objectContaining({ userId: USER })
    )
  })
  ...
  it('events are queried per calendar with from/to window', async () => {
    ...
    expect(events.list).toHaveBeenCalledWith(
      ORG,
      expect.objectContaining({ calendarId: 'cal_1', from: FROM, to: TO })
    )
  })
  ...
  it('task with null dates does not appear in window-filtered tasks', async () => {
    vi.mocked(tasks.list).mockResolvedValue({
      data: [taskRow({ id: 'task_null', startAt: null, dueAt: null })],
      hasMore: false,
    } as never)
    const result = (await service.retrieve(ORG, USER, FROM, TO)) as {
      tasks: unknown[]
  ...
  it('task that is both assigned and owned appears once in the task list', async () => {
    vi.mocked(tasks.list).mockResolvedValue({
      data: [taskRow({ id: 'task_both', dueAt: FROM + 100 })],
      hasMore: false,
    } as never)
    const result = (await service.retrieve(ORG, USER, FROM, TO)) as {
      tasks: { id: string }[]
  ...
  it('events spanning multiple visible calendars are aggregated without duplication', async () => {
    ...
    vi.mocked(events.list)
      .mockResolvedValueOnce({
        data: [eventRow({ id: 'event_shared', startAt: FROM + 1000 })],
        hasMore: false,
      } as never)
      .mockResolvedValueOnce({
        data: [eventRow({ id: 'event_shared', startAt: FROM + 1000 })],
  ...
  it('paginates task collection across multiple pages', async () => {
    vi.mocked(tasks.list)
      .mockResolvedValueOnce({
        data: [taskRow({ id: 'task_page1', dueAt: FROM + 100 })],
        hasMore: true,
      } as never)
      .mockResolvedValueOnce({
        data: [taskRow({ id: 'task_page2', dueAt: FROM + 200 })],
        hasMore: false,
      } as never)
    const result = (await service.retrieve(ORG, USER, FROM, TO)) as {
      tasks: { id: string }[]
    ...
    expect(tasks.list).toHaveBeenNthCalledWith(
      1,
      ORG,
    ...
    expect(tasks.list).toHaveBeenNthCalledWith(
      2,
      ORG,
      expect.objectContaining({ startingAfter: 'task_page1' })
    )
  })
  ...
  it('returns the full my_work shape with window and user fields', async () => {
    const result = (await service.retrieve(ORG, USER, FROM, TO)) as Record<
      string,
      unknown
    >
  ...
  it('paginates events within a calendar across multiple pages', async () => {
    ...
    vi.mocked(events.list)
      .mockResolvedValueOnce({
        data: [eventRow({ id: 'event_page1', startAt: FROM + 100 })],
        hasMore: true,
      } as never)
      .mockResolvedValueOnce({
        data: [eventRow({ id: 'event_page2', startAt: FROM + 200 })],
        hasMore: false,
      } as never)
    const result = (await service.retrieve(ORG, USER, FROM, TO)) as {
      events: { id: string }[]
    ...
    expect(events.list).toHaveBeenNthCalledWith(
      1,
      ORG,
      expect.objectContaining({ calendarId: 'cal_1' })
    )
    expect(events.list).toHaveBeenNthCalledWith(
      2,
      ORG,
  ...
  it('paginates reminders across multiple pages and returns them all', async () => {
    vi.mocked(reminders.list)
      .mockResolvedValueOnce({
        data: [reminderRow({ id: 'rem_page1', remindAt: FROM + 100 })],
        hasMore: true,
      } as never)
      .mockResolvedValueOnce({
        data: [reminderRow({ id: 'rem_page2', remindAt: FROM + 200 })],
        hasMore: false,
      } as never)
    const result = (await service.retrieve(ORG, USER, FROM, TO)) as {
      reminders: { id: string }[]
    }
    expect(result.reminders.map((r) => r.id)).toEqual([
      'rem_page1',
      'rem_page2',
    ])
    expect(reminders.list).toHaveBeenNthCalledWith(
      2,
      ORG,
      expect.objectContaining({ startingAfter: 'rem_page1' })
    )
  })
})
