...
import { createReminderBodySchema, deleteReminderBodySchema, listRemindersQuerySchema, organizationParamsSchema, reminderParamsSchema, updateReminderBodySchema } from './reminders.schemas.js'
...
export async function listReminders(req: Request, res: Response) {
  ...
  const context = query.context_service ? { service: query.context_service, resource: query.context_resource!, id: query.context_id! } : undefined
  ...
  return sendWorkList(res, result, `/v1/organizations/${organizationId}/reminders`)
}
...
export async function createReminder(req: Request, res: Response) {
  const { organizationId } = organizationParamsSchema.parse(req.params)
  return sendWorkResult(res, await service.create(organizationId, createReminderBodySchema.parse(req.body)), 201)
}
export async function updateReminder(req: Request, res: Response) {
  const { organizationId, reminderId } = reminderParamsSchema.parse(req.params)
  const result = await service.update(organizationId, reminderId, updateReminderBodySchema.parse(req.body))
  if (!result) return sendWorkError(res, 'work/reminder-not-found')
  return sendWorkResult(res, result)
}
