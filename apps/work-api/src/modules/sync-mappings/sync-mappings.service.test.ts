...
const connection = { id: 'sync_conn_mandeville_1', organizationId: 'org_kingston_1' }
...
describe('Work sync-mappings service', () => {
  it('create stores local↔remote id pair with iCalUid', async () => {
    ...
    const result = await service.create('org_kingston_1', connection.id, { resourceType: 'EVENT', localId: 'event_kingston_1', remoteId: 'remote_abc_123', iCalUid: 'event_kingston_1@work.876' })
    expect(result).toEqual(expect.objectContaining({ localId: 'event_kingston_1', remoteId: 'remote_abc_123', iCalUid: 'event_kingston_1@work.876' }))
    expect(repository.create).toHaveBeenCalledWith(expect.objectContaining({ connectionId: connection.id, resourceType: 'EVENT', localId: 'event_kingston_1', remoteId: 'remote_abc_123' }))
  })

  it('create with ETag and contentHash persists them', async () => {
    vi.mocked(repository.create).mockResolvedValue(row({ remoteEtag: '"etag-99"', contentHash: 'hash_xyz' }) as never)
    await service.create('org_kingston_1', connection.id, { resourceType: 'TASK', localId: 'task_1', remoteId: 'remote_1', remoteEtag: '"etag-99"', contentHash: 'hash_xyz' })
    expect(repository.create).toHaveBeenCalledWith(expect.objectContaining({ remoteEtag: '"etag-99"', contentHash: 'hash_xyz' }))
  })

  it('list returns mappings for connection', async () => {
    vi.mocked(repository.list).mockResolvedValue([row({ id: 'map_a' }), row({ id: 'map_b' })] as never)
    const result = await service.list('org_kingston_1', connection.id) as { data: unknown[] }
  ...
  it('update changes remoteEtag and contentHash and touches lastSyncedAt', async () => {
    vi.mocked(repository.retrieve).mockResolvedValue(row({ id: 'map_1' }) as never)
    vi.mocked(repository.update).mockResolvedValue(row({ remoteEtag: '"etag-new"', contentHash: 'hash_new' }) as never)
    await service.update('org_kingston_1', connection.id, 'map_1', { remoteEtag: '"etag-new"', contentHash: 'hash_new' })
    expect(repository.update).toHaveBeenCalledWith('map_1', expect.objectContaining({ remoteEtag: '"etag-new"', contentHash: 'hash_new', lastSyncedAt: expect.any(Date) }))
  })

  it('update changes iCalUid', async () => {
    vi.mocked(repository.retrieve).mockResolvedValue(row() as never)
    vi.mocked(repository.update).mockResolvedValue(row({ iCalUid: 'new_uid@work.876' }) as never)
    await service.update('org_kingston_1', connection.id, 'map_1', { iCalUid: 'new_uid@work.876' })
    expect(repository.update).toHaveBeenCalledWith('map_1', expect.objectContaining({ iCalUid: 'new_uid@work.876' }))
  })

  it('update returns null when mapping not found and never updates', async () => {
    vi.mocked(repository.retrieve).mockResolvedValue(null)
    const result = await service.update('org_kingston_1', connection.id, 'missing', { remoteEtag: '"x"' })
  ...
  it('remove deletes mapping', async () => {
    vi.mocked(repository.retrieve).mockResolvedValue(row() as never)
    vi.mocked(repository.remove).mockResolvedValue({ object: 'sync_mapping', id: 'map_1', deleted: true } as never)
    const result = await service.remove('org_kingston_1', connection.id, 'map_1')
  ...
  it('remove returns null when mapping not found and never removes', async () => {
    vi.mocked(repository.retrieve).mockResolvedValue(null)
    const result = await service.remove('org_kingston_1', connection.id, 'missing')
    expect(result).toBeNull()
  ...
  it('create returns null when connection not found and never creates', async () => {
    ...
    const result = await service.create('org_kingston_1', 'missing_conn', { resourceType: 'EVENT', localId: 'event_1', remoteId: 'remote_1' })
    expect(result).toBeNull()
  ...
  it('create returns connection tenant error and never creates when connection in other tenant', async () => {
    ...
    const result = await service.create('org_kingston_1', connection.id, { resourceType: 'EVENT', localId: 'event_1', remoteId: 'remote_1' })
    expect(result).toEqual(err)
    expect(repository.create).not.toHaveBeenCalled()
  })

  it('serializes the mapping with object discriminator and Unix timestamps', async () => {
    vi.mocked(repository.list).mockResolvedValue([row({ id: 'map_1' })] as never)
    const result = await service.list('org_kingston_1', connection.id) as { data: unknown[] }
  ...
  it('update refreshes lastSyncedAt even for a metadata-only change', async () => {
    vi.mocked(repository.retrieve).mockResolvedValue(row({ id: 'map_1' }) as never)
    vi.mocked(repository.update).mockResolvedValue(row({ id: 'map_1', remoteEtag: '"etag-2"' }) as never)
    await service.update('org_kingston_1', connection.id, 'map_1', { remoteEtag: '"etag-2"' })
    expect(repository.update).toHaveBeenCalledWith(
      'map_1',
      expect.objectContaining({ remoteEtag: '"etag-2"', lastSyncedAt: expect.any(Date) })
    )
  })

  it('update clears the iCalUid when null is supplied', async () => {
    vi.mocked(repository.retrieve).mockResolvedValue(row({ id: 'map_1', iCalUid: 'event_kingston_1@work.876' }) as never)
    vi.mocked(repository.update).mockResolvedValue(row({ id: 'map_1', iCalUid: null }) as never)
    await service.update('org_kingston_1', connection.id, 'map_1', { iCalUid: null })
    expect(repository.update).toHaveBeenCalledWith(
      'map_1',
