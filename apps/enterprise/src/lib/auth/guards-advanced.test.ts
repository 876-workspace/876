...
describe('requireSession — diff: /access-denied not /register (enterprise gate)', () => {
  it('never redirects to legacy /register for consumer realm', async () => {
    ...
    expect(mocks.redirect).not.toHaveBeenCalledWith(expect.stringContaining('/register'))
  })

  it('redirect target is exactly /access-denied (no query pollution)', async () => {
    mocks.getAuthSession.mockResolvedValue(signedSession(sessionUser({ realm: 'consumer', crossRealm: false })))
    try { await requireSession('/protected') } catch {}
    const path = mocks.redirect.mock.calls[0]?.[0] as string
  ...
  it('allows enterprise realm regardless of crossRealm flag', async () => {
    for (const crossRealm of [false, true] as const) {
      vi.clearAllMocks()
      mocks.redirect.mockImplementation((path: string) => { throw redirectError(path) })
      mocks.getAuthSession.mockResolvedValue(signedSession(sessionUser({ realm: 'enterprise', crossRealm })))
  ...
  ])('encodes returnTo "%s" as "%s" for unauthenticated', async (returnTo, expected) => {
    mocks.getAuthSession.mockResolvedValue({ user: null } as never)
    await expect(requireSession(returnTo)).rejects.toMatchObject({ path: expected })
  })

  it('propagates NEXT_REDIRECT digest for consumer block', async () => {
    mocks.getAuthSession.mockResolvedValue(signedSession(sessionUser({ realm: 'consumer', crossRealm: false })))
    try { await requireSession('/') } catch (e: unknown) {
      expect((e as { digest?: string }).digest).toBe('NEXT_REDIRECT;/access-denied')
    }
  })
  ...
  it('concurrent calls are isolated — Promise.all', async () => {
    mocks.getAuthSession
      .mockResolvedValueOnce(signedSession(sessionUser({ realm: 'enterprise', crossRealm: false })))
      .mockResolvedValueOnce(signedSession(sessionUser({ realm: 'consumer', crossRealm: false })))
      .mockResolvedValueOnce(signedSession(sessionUser({ realm: 'enterprise', crossRealm: false })))
    const results = await Promise.allSettled([requireSession('/a'), requireSession('/b'), requireSession('/c')])
  ...
  it('returns the session user object verbatim (no stripping)', async () => {
    ...
    expect(out).toEqual(expect.objectContaining({ id: 'user_42', realm: 'enterprise' }))
  })
...
describe('requireActiveUser — diff: /login?returnTo=%2F not /register', () => {
  ...
  it('is relative (not absolute) even though suspended later is absolute', async () => {
    mocks.usersRetrieve.mockResolvedValue({ data: null, error: null })
    try { await requireActiveUser('missing') } catch (e: unknown) {
  ...
  it.each([
    ...
    mocks.usersRetrieve.mockResolvedValue({ data: { id: 'user_1', email: 'a@b.co', status, banned: false, first_name: 'A', last_name: null, avatar: null }, error: null })
    await expect(requireActiveUser('user_1')).rejects.toMatchObject({ path: expect.stringContaining('/suspended') })
    const path = mocks.redirect.mock.calls[0]?.[0] as string
  ...
  it('active unbanned returns user with snake→camel mapping', async () => {
    mocks.usersRetrieve.mockResolvedValue({ data: { id: 'user_1', email: 'a@b.co', status: 'active', banned: false, first_name: 'Jane', last_name: 'Doe', avatar: 'https://cdn/img.png' }, error: null })
    const user = await requireActiveUser('user_1')
  ...
  it('throws on platform error (does not swallow outage)', async () => {
    mocks.usersRetrieve.mockResolvedValue({ data: null, error: { code: 'internal', message: 'db down' } })
  ...
  it('throws when user lookup returns error envelope with code internal for workos fallback', async () => {
    mocks.usersRetrieve.mockResolvedValueOnce({ data: null, error: { code: 'internal', message: 'boom' } })
...
describe('requireOrgMembership — diff: "/" gate and slug-encoded login', () => {
  function activeUser() { return { data: { id: 'user_1', email: 'a@b.co', status: 'active', banned: false, first_name: 'A', last_name: null, avatar: null }, error: null } }
  ...
  it.each(['acme','my-org','acme_123','a'])('unknown user "%s" encodes returnTo', async (slug) => {
    mocks.usersRetrieve.mockResolvedValue({ data: null, error: null })
    await expect(requireOrgMembership('unknown', slug)).rejects.toMatchObject({ path: `/login?returnTo=%2F${slug}` })
    vi.clearAllMocks()
    mocks.redirect.mockImplementation((path: string) => { throw redirectError(path) })
    mocks.usersRetrieve.mockResolvedValue({ data: null, error: null } as never)
    mocks.listRouting.mockResolvedValue({ data: { data: [] }, error: null } as never)
  })

  it('slug with slash is encoded', async () => {
    mocks.usersRetrieve.mockResolvedValue({ data: null, error: null })
    await expect(requireOrgMembership('unknown','acme/profile')).rejects.toMatchObject({ path: expect.stringContaining(encodeURIComponent('/acme/profile')) })
  })

  it('propagates platform error from listRouting', async () => {
    mocks.usersRetrieve.mockResolvedValue(activeUser() as never)
    mocks.listRouting.mockResolvedValue({ data: null, error: { code: 'internal', message: 'outage' } } as never)
  ...
  it('concurrent checks isolated', async () => {
    mocks.usersRetrieve.mockResolvedValue(activeUser() as never)
    mocks.listRouting
      .mockResolvedValueOnce({ data: { data: [membershipRow({ slug: 'acme' })] }, error: null })
      .mockResolvedValueOnce({ data: { data: [] }, error: null })
    const [a,b] = await Promise.allSettled([requireOrgMembership('user_1','acme'), requireOrgMembership('user_1','other')])
    expect(a.status).toBe('fulfilled')
    expect(b.status).toBe('rejected')
    if (b.status==='rejected') expect((b.reason as { path: string }).path).toBe('/')
  })
...
describe('findAuthRoutingUser — mapping & resilience (advanced)', () => {
  ...
  it.each([[null as unknown as string, 'a@b.co'], ['user_1', null as unknown as string], ['' as unknown as string, 'a@b.co']])('returns null when id/email falsy (%s,%s)', async (id, email) => {
    mocks.usersRetrieve.mockResolvedValue({ data: { id, email, status:'active', banned:false, first_name:'A', last_name:null, avatar:null }, error:null })
    await expect(findAuthRoutingUser('user_1')).resolves.toBeNull()
  })

  it('defaults status to active when null', async () => {
    mocks.usersRetrieve.mockResolvedValue({ data: { id:'user_1', email:'a@b.co', status:null, banned: 0 as unknown as boolean, first_name:null, last_name:null, avatar:null }, error:null })
    const user = await findAuthRoutingUser('user_1')
  ...
  it('throws on fallback error envelope', async () => {
    mocks.usersRetrieve.mockResolvedValueOnce({ data:null, error:null }).mockResolvedValueOnce({ data:null, error:{ code:'internal', message:'boom' } })
  ...
  it('maps snake_case all fields', async () => {
    ...
    expect(u).toEqual(expect.objectContaining({ firstName:'First', lastName:'Last', avatar:'https://img' }))
  })

  it('falls back to workosId when primary null', async () => {
    mocks.usersRetrieve.mockResolvedValueOnce({ data:null, error:null }).mockResolvedValueOnce({ data:{ id:'user_1', email:'a@b.co', status:'active', banned:false, first_name:'A', last_name:null, avatar:null }, error:null })
...
describe('hasOrgPermission — pure & boundary (goldbergyoni: no logic in tests)', () => {
  ...
  it('is case-sensitive', () => { expect(hasOrgPermission({ permissions:['Org:Read'] }, 'org:read')).toBe(false) })
  it('does not mutate input', () => {
    const perms = ['org:read']
    hasOrgPermission({ permissions: perms }, 'org:read')
    expect(perms).toEqual(['org:read'])
  })
  it('handles duplicates', () => { expect(hasOrgPermission({ permissions:['org:read','org:read'] }, 'org:read')).toBe(true) })
  it('handles empty permission string', () => { expect(hasOrgPermission({ permissions:[''] }, '')).toBe(true) })
...
describe('requireOrgPermission — /<slug> gate', () => {
  ...
  it('redirects to /<slug> when permission missing', async () => {
    mocks.usersRetrieve.mockResolvedValue(activeUser() as never)
    mocks.listRouting.mockResolvedValue({ data:{ data:[{ ...membershipRow({ slug:'acme' }), permissions:['org:read'] }] }, error:null })
    await expect(requireOrgPermission('user_1','acme','members:read')).rejects.toMatchObject({ path:'/acme' })
    expect(mocks.redirect).toHaveBeenCalledWith('/acme')
  })
  it('propagates missing membership as "/" before permission check', async () => {
    mocks.usersRetrieve.mockResolvedValue(activeUser() as never)
    mocks.listRouting.mockResolvedValue({ data:{ data:[] }, error:null })
    await expect(requireOrgPermission('user_1','acme','org:read')).rejects.toMatchObject({ path:'/' })
  })
  it('passes when permission present (no redirect)', async () => {
    mocks.usersRetrieve.mockResolvedValue(activeUser() as never)
    mocks.listRouting.mockResolvedValue({ data:{ data:[membershipRow({ slug:'acme' })] }, error:null })
    await expect(requireOrgPermission('user_1','acme','org:read')).resolves.toBeDefined()
    expect(mocks.redirect).not.toHaveBeenCalled()
  })
})
...
describe('resolvePrimaryOrganizationPath — relative & filtering', () => {
  it('prefers first active in order', async () => {
    mocks.listRouting.mockResolvedValue({ data:{ data:[membershipRow({ slug:'first' }), membershipRow({ slug:'second' })] }, error:null })
    await expect(resolvePrimaryOrganizationPath('user_1')).resolves.toBe('/first/profile')
  })
  it('skips blocked membership and blocked org', async () => {
    mocks.listRouting.mockResolvedValue({ data:{ data:[membershipRow({ slug:'blocked', status:'blocked' }), membershipRow({ slug:'acme' })] }, error:null })
    await expect(resolvePrimaryOrganizationPath('user_1')).resolves.toBe('/acme/profile')
  })
  it('skips org with blocked status', async () => {
    mocks.listRouting.mockResolvedValue({ data:{ data:[membershipRow({ slug:'acme', orgStatus:'blocked' }), membershipRow({ slug:'other' })] }, error:null })
    await expect(resolvePrimaryOrganizationPath('user_1')).resolves.toBe('/other/profile')
  })
  it('returns null when none active', async () => {
    mocks.listRouting.mockResolvedValue({ data:{ data:[membershipRow({ slug:'a', status:'blocked' }), membershipRow({ slug:'b', orgStatus:'inactive' })] }, error:null })
    await expect(resolvePrimaryOrganizationPath('user_1')).resolves.toBeNull()
  })
  it('returns relative URL with slug verbatim', async () => {
    mocks.listRouting.mockResolvedValue({ data:{ data:[membershipRow({ slug:'my-org' })] }, error:null })
    const path = await resolvePrimaryOrganizationPath('user_1')
...
describe('resolveHomePathForUser — onboarding fallback never absolute (diff)', () => {
  it('concurrent callers agree on /onboarding', async () => {
    mocks.listRouting.mockResolvedValue({ data:{ data:[] }, error:null })
    const paths = await Promise.all([resolveHomePathForUser('user_1'), resolveHomePathForUser('user_1'), resolveHomePathForUser('user_1')])
...
describe('getEnabledEnterpriseFeatureSlugs — observability & dedup', () => {
  it('deduplicates via Set', async () => {
    mocks.featuresEvaluate.mockResolvedValue({ data:{ data:[{ slug:'billing' },{ slug:'billing' }] }, error:null })
    const slugs = await getEnabledEnterpriseFeatureSlugs('org_1')
  ...
  it('calls evaluate with ENTERPRISE_APP_SLUG and organizationId', async () => {
    ...
    expect(mocks.featuresEvaluate).toHaveBeenCalledWith(expect.objectContaining({ organizationId:'org_99', appSlug: expect.any(String) }))
  })
  it('when org undefined calls with undefined', async () => {
    mocks.featuresEvaluate.mockResolvedValue({ data:{ data:[] }, error:null })
    await getEnabledEnterpriseFeatureSlugs()
    expect(mocks.featuresEvaluate).toHaveBeenCalledWith(expect.objectContaining({ organizationId: undefined }))
  })
  it('Sentry capture level error and category', async () => {
    mocks.featuresEvaluate.mockResolvedValue({ data:null, error:{ code:'internal', message:'outage' } })
    try { await getEnabledEnterpriseFeatureSlugs('org_1') } catch {}
    expect(mocks.captureMessage).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ level:'error', tags: expect.objectContaining({ category:'feature_flags' }) }))
  })
  it('Sentry extra includes errorCode and call', async () => {
    mocks.featuresEvaluate.mockResolvedValue({ data:null, error:{ code:'rate_limited', message:'slow' } })
    try { await getEnabledEnterpriseFeatureSlugs('org_1') } catch {}
    const call = mocks.captureMessage.mock.calls[0]?.[1] as { extra?: Record<string,unknown> }
    expect(call?.extra).toEqual(expect.objectContaining({ errorCode:'rate_limited', call:'features.evaluate' }))
  })
  ...
  it('concurrent isolated', async () => {
    mocks.featuresEvaluate.mockResolvedValueOnce({ data:{ data:[{ slug:'a' }] }, error:null }).mockResolvedValueOnce({ data:{ data:[{ slug:'b' }] }, error:null })
    const [a,b] = await Promise.all([getEnabledEnterpriseFeatureSlugs('org_1'), getEnabledEnterpriseFeatureSlugs('org_2')])
...
describe('requireEnterpriseFeature — redirect contract', () => {
  ...
  it('custom redirect verbatim', async () => {
    mocks.featuresEvaluate.mockResolvedValue({ data:{ data:[] }, error:null })
    await expect(requireEnterpriseFeature('billing','org_1','/custom-path')).rejects.toMatchObject({ path:'/custom-path' })
  })
...
describe('contract: all redirects carry NEXT_REDIRECT digest', () => {
  it.each([
    ['requireSession consumer', async () => { mocks.getAuthSession.mockResolvedValue(signedSession(sessionUser({ realm:'consumer', crossRealm:false }))); await requireSession('/') }],
    ['requireActiveUser unknown', async () => { mocks.usersRetrieve.mockResolvedValue({ data:null, error:null }); await requireActiveUser('missing') }],
    ['requireOrgMembership missing', async () => { mocks.usersRetrieve.mockResolvedValue({ data:{ id:'user_1', email:'a@b.co', status:'active', banned:false, first_name:'A', last_name:null, avatar:null }, error:null }); mocks.listRouting.mockResolvedValue({ data:{ data:[] }, error:null }); await requireOrgMembership('user_1','acme') }],
...
describe('guards — goldbergyoni anti-flaky & boundary extras', () => {
  it('requireSession with undefined realm blocks (treated as consumer)', async () => {
    mocks.getAuthSession.mockResolvedValue(signedSession(sessionUser({ realm: undefined as unknown as string, crossRealm: false })))
    await expect(requireSession('/')).rejects.toMatchObject({ path: '/access-denied' })
  })
  it('requireSession with empty string realm blocks', async () => {
    mocks.getAuthSession.mockResolvedValue(signedSession(sessionUser({ realm: '' as unknown as string, crossRealm: false })))
    await expect(requireSession('/')).rejects.toMatchObject({ path: '/access-denied' })
  })
  it('findAuthRoutingUser handles avatar null vs undefined mapping', async () => {
    mocks.usersRetrieve.mockResolvedValue({ data:{ id:'user_1', email:'a@b.co', status:'active', banned:false, first_name:null, last_name:null, avatar: undefined as unknown as string }, error:null })
    const u = await findAuthRoutingUser('user_1')
    expect(u?.avatar).toBeNull()
  })
  it('resolvePrimaryOrganizationPath skips orgStatus empty string (not active)', async () => {
    mocks.listRouting.mockResolvedValue({ data:{ data:[membershipRow({ slug:'acme', orgStatus: '' as unknown as string }), membershipRow({ slug:'other' })] }, error:null })
    // empty orgStatus != 'active' so skipped, returns other
    await expect(resolvePrimaryOrganizationPath('user_1')).resolves.toBe('/other/profile')
  })
  it('getEnabledEnterpriseFeatureSlugs handles null data array? throws', async () => {
    mocks.featuresEvaluate.mockResolvedValue({ data:{ data: null as unknown as { slug:string }[] }, error:null })
    await expect(getEnabledEnterpriseFeatureSlugs('org_1')).rejects.toBeDefined()
  })
})
