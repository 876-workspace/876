...
vi.mock('@/lib/auth/guards', async (importOriginal) => {
  ...
  return { ...actual, requireSession: mocks.requireSession, findAuthRoutingUser: mocks.findAuthRoutingUser, resolveHomePathForUser: mocks.resolveHomePathForUser }
})

import RootPage from './page'

function redirectErr(path: string) {
  const e = Object.assign(new Error(path), { path, digest: `NEXT_REDIRECT:${path}` })
  throw e
}
...
describe('RootPage — home resolver (goldbergyoni AAA, isolated)', () => {
  ...
  it('diff: redirects to /login?returnTo=%2F when session user no longer local (not /register)', async () => {
    ...
    expect(mocks.redirect.mock.calls[0]?.[0] as string).toBe('/login?returnTo=%2F')
  })
  it('redirect target for missing user is exactly /login?returnTo=%2F (encoded, relative)', async () => {
    mocks.findAuthRoutingUser.mockResolvedValue(null)
    try { await RootPage() } catch {}
  ...
  it('resolves auth routing user with sessionUser.id', async () => {
    ...
    mocks.findAuthRoutingUser.mockResolvedValue({ id: 'local_123', email: 'x@y.co' } as never)
    mocks.resolveHomePathForUser.mockResolvedValue('/acme/profile')
    try { await RootPage() } catch {}
  ...
  it('short-circuits when requireSession redirects', async () => {
    mocks.requireSession.mockImplementation(() => { throw redirectErr('/login?returnTo=%2F') })
    await expect(RootPage()).rejects.toMatchObject({ path: '/login?returnTo=%2F' })
  ...
  it('redirect carries NEXT_REDIRECT digest', async () => {
    try { await RootPage() } catch (e: unknown) {
      expect((e as { digest?: string }).digest).toBe(`NEXT_REDIRECT:${(e as { path: string }).path}`)
    }
  })
  ...
  it('concurrent calls isolated', async () => {
    mocks.resolveHomePathForUser.mockResolvedValueOnce('/acme/profile' as never).mockResolvedValueOnce('/other/profile' as never)
    const [a,b] = await Promise.allSettled([RootPage(), RootPage()])
    const paths = [a,b].map(r => (r as PromiseRejectedResult).reason.path as string)
  ...
  it.each([['/acme/profile','/acme/profile'],['/register','/register']])('forwards resolveHomePathForUser "%s"', async (path) => {
