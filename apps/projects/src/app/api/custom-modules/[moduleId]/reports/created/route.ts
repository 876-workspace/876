...
import { createdReportCsv } from '@/lib/custom-modules/record-form-helpers'
...
export async function GET(request: NextRequest, { params }: Props) {
  ...
  if (parsedQuery.data.format === 'csv') {
    return new Response(createdReportCsv(result.data), {
      headers: {
        'content-type': 'text/csv; charset=utf-8',
        'content-disposition': `attachment; filename="${result.data.moduleKey}-created.csv"`,
      },
    })
  }
