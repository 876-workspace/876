...
type Context = { params: Promise<{ projectId: string; budgetId: string }> }
...
function errorStatus(code: string): 400 | 404 {
  return code === 'projects/budget-not-found' ||
...
export async function GET(_request: Request, { params }: Context) {
  ...
  const result = await projects.budgets.retrieve(
    auth.orgId,
...
export async function PATCH(request: Request, { params }: Context) {
  ...
  const { projectId, budgetId } = await params
  const result = await projects.budgets.update(
    auth.orgId,
    decodeURIComponent(projectId),
    decodeURIComponent(budgetId),
    parsed.data
...
export async function DELETE(_request: Request, { params }: Context) {
  ...
  const { projectId, budgetId } = await params
  const result = await projects.budgets.delete(
    auth.orgId,
    decodeURIComponent(projectId),
    decodeURIComponent(budgetId)
  )
