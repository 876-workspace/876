import { ResourceToolbar } from '@876/ui/resource-toolbar'
import type { Metadata } from 'next'
import { Suspense } from 'react'

import { PageBreadcrumb } from '@/components/page-breadcrumb'
import { EditCycleData } from '@/features/projects/components/edit-cycle-data'
import {
  requireAppAccess,
  requireProjectsContext,
} from '@/lib/auth/require-projects-context'

type Props = { params: Promise<{ cycleId: string }> }

export const metadata: Metadata = { title: 'Edit cycle' }

export default async function EditCyclePage({ params }: Props) {
  await requireAppAccess({ module: 'projects', permission: 'projects.edit' })
  const { orgId } = await requireProjectsContext()
  const { cycleId } = await params

  return (
    <div className="px-4 pt-5 pb-8 sm:px-6 lg:px-8">
      <PageBreadcrumb
        href={`/cycles/${encodeURIComponent(cycleId)}`}
        label="Cycle"
        className="mb-4"
      />
      <ResourceToolbar title="Edit cycle" />
      <Suspense
        fallback={<div className="876-card h-96 max-w-3xl animate-pulse" />}
      >
        <EditCycleData orgId={orgId} cycleId={cycleId} />
      </Suspense>
    </div>
  )
}
