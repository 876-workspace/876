...
const { prisma } = vi.hoisted(() => ({
  prisma: {
    organizationRole: { findMany: vi.fn(), findFirst: vi.fn(), create: vi.fn() },
    app: { findFirst: vi.fn() },
    subscription: { findFirst: vi.fn(), create: vi.fn(), update: vi.fn(), findMany: vi.fn() },
    ...
    $transaction: vi.fn(async (cb: (tx: unknown) => unknown) => cb({ subscriptionItem: { findFirst: vi.fn().mockResolvedValue(null), create: vi.fn().mockResolvedValue({}) } })),
    provisioningManifestRevision: { findFirst: vi.fn().mockResolvedValue(null) },
    provisioningManifest: { findFirst: vi.fn().mockResolvedValue(null) },
  },
}))

vi.mock('@/db/client', () => ({ prisma, disconnectDb: vi.fn(), pingDb: vi.fn() }))
...
vi.mock('../finance-provisioning.repository', () => ({ createFinanceProvisioningRepository: vi.fn(() => ({ marker: 'repo' })) }))
...
const { DEFAULT_ORG_ROLES, defaultPermissionsForRoleName } = await import('@/platform/permissions')
...
function roleRow(overrides: Partial<{ id: string; name: string; permissions: string[] }> = {}) {
  return {
...
beforeEach(() => {
  ...
  prisma.organizationRole.create.mockImplementation(({ data }: { data: Record<string, unknown> }) => Promise.resolve({ ...data, description: (data.description as string) ?? null }))
  prisma.app.findFirst.mockImplementation(({ where }: { where: { slug: string } }) => Promise.resolve(appRow(where.slug)))
  ...
  prisma.organization.findUnique.mockResolvedValue({ id: ORG, name: 'Test Org' })
  ensureAppReady.mockResolvedValue({ ready: true, financeRequired: false, eventIds: [] })
})

// ──────────────────────────────────────────────────────────────
// Contract invariants — the diff's core guarantee: exactly 2 default apps, no Invoice.
// These tests lock the product decision so it cannot silently drift back.
// ──────────────────────────────────────────────────────────────
describe('provisioning contract invariants', () => {
  it('exposes exactly two default app slugs', () => {
    expect(DEFAULT_ORG_APP_SLUGS).toHaveLength(2)
  })

  it('DEFAULT_ORG_APP_SLUGS is Enterprise + Billing in declaration order', () => {
    expect([...DEFAULT_ORG_APP_SLUGS]).toEqual([ENTERPRISE_APP_SLUG, BILLING_APP_SLUG])
  })

  it('does not include 876-invoice', () => {
    expect([...DEFAULT_ORG_APP_SLUGS]).not.toContain('876-invoice')
    expect((DEFAULT_ORG_APP_SLUGS as readonly string[])).not.toContain('876-invoice')
  })
  ...
  it('DEFAULT_ORG_APP_SLUGS is readonly tuple (invoice cannot be added without type error)', () => {
    const copy = [...DEFAULT_ORG_APP_SLUGS]
    ;(copy as string[]).push('876-invoice')
    expect(copy).toContain('876-invoice')
    expect([...DEFAULT_ORG_APP_SLUGS]).not.toContain('876-invoice')
    expect(DEFAULT_ORG_APP_SLUGS).toHaveLength(2)
  })

  it('bootstrap org docs mention Enterprise and Billing but not Invoice', () => {
...
describe('seedDefaultRoles — advanced', () => {
  it('creates every default role via repository and returns map keyed by name', async () => {
    ...
    expect(Object.keys(roles).sort()).toEqual(DEFAULT_ORG_ROLES.map((r) => r.name).sort())
    expect(prisma.organizationRole.create).toHaveBeenCalledTimes(DEFAULT_ORG_ROLES.length)
  })

  it('is idempotent: second seeding creates zero new rows when all roles exist', async () => {
    const existing = DEFAULT_ORG_ROLES.map((d) => roleRow({ name: d.name, permissions: [...d.permissions], id: `rol_${d.name}` }))
    prisma.organizationRole.findMany.mockResolvedValueOnce([]).mockResolvedValueOnce(existing)
  ...
  it('keeps custom permissions when org has already customised a system role', async () => {
    const custom = roleRow({ name: 'owner', permissions: ['custom:permission'], id: 'rol_custom' })
    prisma.organizationRole.findMany.mockResolvedValue([custom])
    const roles = await seedDefaultRoles(ORG, NOW)
    expect(roles.owner!.permissions).toEqual(['custom:permission'])
    expect(prisma.organizationRole.create).not.toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ name: 'owner' }) }))
  })
  ...
  it('creates missing roles when only a subset exists', async () => {
    ...
    await seedDefaultRoles(ORG, NOW)
    expect(prisma.organizationRole.create).toHaveBeenCalledTimes(DEFAULT_ORG_ROLES.length - 1)
    const createdNames = vi.mocked(prisma.organizationRole.create).mock.calls.map((c) => (c[0] as { data: { name: string } }).data.name)
    expect(createdNames).not.toContain(one.name)
  })

  it('seeds roles as system roles with BigInt timestamps', async () => {
    await seedDefaultRoles(ORG, NOW)
    const firstCall = prisma.organizationRole.create.mock.calls[0]?.[0] as { data: Record<string, unknown> }
...
describe('ensureOrgAppSubscriptions — durable provisioning', () => {
  it('provisions exactly Enterprise and Billing for a fresh org', async () => {
    const result = await ensureOrgAppSubscriptions(ORG)
    expect(result.provisioned).toEqual([`app_${ENTERPRISE_APP_SLUG}`, `app_${BILLING_APP_SLUG}`])
    expect(result.appIds).toEqual([`app_${ENTERPRISE_APP_SLUG}`, `app_${BILLING_APP_SLUG}`])
  ...
  it('is idempotent: second call provisions nothing but returns same appIds', async () => {
    ...
    prisma.subscription.findFirst.mockResolvedValue({ id: 'sub_existing', status: 'active', subscriptionItems: [{ id: 'si_1' }] } as unknown as never)
    const second = await ensureOrgAppSubscriptions(ORG)
    expect(second.provisioned).toEqual([])
    expect(second.appIds).toHaveLength(2)
  })
  ...
  it('attaches sourceAppId when it is not already a default', async () => {
    const result = await ensureOrgAppSubscriptions(ORG, { sourceAppId: 'app_couriers' })
    expect(result.appIds).toContain('app_couriers')
    expect(result.provisioned).toContain('app_couriers')
    expect(prisma.subscription.create).toHaveBeenCalledTimes(3)
  })

  it('does not duplicate when sourceAppId equals Enterprise', async () => {
    const result = await ensureOrgAppSubscriptions(ORG, { sourceAppId: `app_${ENTERPRISE_APP_SLUG}` })
    expect(result.appIds).toEqual([`app_${ENTERPRISE_APP_SLUG}`, `app_${BILLING_APP_SLUG}`])
    expect(prisma.subscription.create).toHaveBeenCalledTimes(2)
  })

  it('does not duplicate when sourceAppId equals Billing', async () => {
    const result = await ensureOrgAppSubscriptions(ORG, { sourceAppId: `app_${BILLING_APP_SLUG}` })
    expect(result.appIds).toHaveLength(2)
  })

  it('omits a default app whose app row is missing (partially seeded env) and continues', async () => {
    prisma.app.findFirst.mockImplementation(({ where }: { where: { slug: string } }) => {
      if (where.slug === BILLING_APP_SLUG) return Promise.resolve(null)
  ...
  it('skips already-subscribed org: no create, but still returns appIds', async () => {
    prisma.subscription.findFirst.mockResolvedValue({ id: 'sub_1', status: 'active', subscriptionItems: [{ id: 'si_1' }] } as never)
    const result = await ensureOrgAppSubscriptions(ORG)
  ...
  it('repairs itemless active subscription by attaching default price instead of recreating', async () => {
    prisma.subscription.findFirst.mockResolvedValue({ id: 'sub_itemless', status: 'active', subscriptionItems: [] } as never)
  ...
  it('uses distinct appIds (no duplicates even after retries)', async () => {
    const first = await ensureOrgAppSubscriptions(ORG, { sourceAppId: 'app_extra' })
...
describe('ensureOrgAppsFinanceReady', () => {
  it('calls ensureAppReady for each subscribed app', async () => {
    prisma.subscription.findMany.mockResolvedValue([{ appId: `app_${ENTERPRISE_APP_SLUG}` }, { appId: `app_${BILLING_APP_SLUG}` }] as never)
  ...
  it('uses supplied appIds without querying DB', async () => {
    await ensureOrgAppsFinanceReady(ORG, { appIds: [`app_${ENTERPRISE_APP_SLUG}`] })
    expect(prisma.subscription.findMany).not.toHaveBeenCalled()
    expect(ensureAppReady).toHaveBeenCalledTimes(1)
    expect(ensureAppReady).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ organizationId: ORG, appId: `app_${ENTERPRISE_APP_SLUG}` }))
  })
  ...
  it('propagates finance outage error', async () => {
    prisma.subscription.findMany.mockResolvedValue([{ appId: `app_${BILLING_APP_SLUG}` }] as never)
    ensureAppReady.mockRejectedValueOnce(new Error('finance down'))
    await expect(ensureOrgAppsFinanceReady(ORG, {})).rejects.toThrow('finance down')
  })

  it('runs readiness for source-added app as well', async () => {
    await ensureOrgAppsFinanceReady(ORG, { appIds: [`app_${ENTERPRISE_APP_SLUG}`, `app_${BILLING_APP_SLUG}`, 'app_couriers'] })
...
describe('provisionOrgApps — durable then finance', () => {
  it('provisions two defaults and runs readiness for both', async () => {
    const provisioned = await provisionOrgApps(ORG)
    expect(provisioned).toEqual([`app_${ENTERPRISE_APP_SLUG}`, `app_${BILLING_APP_SLUG}`])
    expect(ensureAppReady).toHaveBeenCalledTimes(2)
  })

  it('durability: subscriptions are written even when readiness throws', async () => {
    ensureAppReady.mockRejectedValueOnce(new Error('provisioning/finance-workspace-unavailable'))
    await expect(provisionOrgApps(ORG)).rejects.toThrow('provisioning/finance-workspace-unavailable')
    expect(prisma.subscription.create).toHaveBeenCalledTimes(2)
  })
  ...
  it('forwards sourceAppId through to durable layer', async () => {
    const provisioned = await provisionOrgApps(ORG, { sourceAppId: 'app_extra' })
    expect(provisioned).toContain('app_extra')
    expect(ensureAppReady).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ appId: 'app_extra' }))
  })
})
...
describe('provisionOrganization — full lifecycle', () => {
  ...
  it('forwards sourceAppId to app provisioning', async () => {
    const enqueue = vi.fn().mockResolvedValue(undefined)
    await provisionOrganization(ORG, NOW, { sourceAppId: 'app_extra', enqueueCustomerEnsure: enqueue })
    expect(prisma.subscription.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ appId: 'app_extra' }) }))
  })
  ...
  it('still provisions durable subscriptions when enqueue throws (best-effort is injected)', async () => {
    ...
    await expect(provisionOrganization(ORG, NOW, { enqueueCustomerEnsure: enqueue })).rejects.toThrow('outbox down')
    expect(prisma.subscription.create).toHaveBeenCalled()
  })
})
...
describe('ensureDefaultContact', () => {
  const USER = { id: 'user_1', firstName: 'Alejandra', lastName: 'Stone', email: 'alejandra@example.com', phone: '+14155550100' }
  ...
  it('creates primary general contact when org has none', async () => {
    ...
    const data = (prisma.orgContact.create.mock.calls[0]?.[0] as { data: Record<string, unknown> }).data
  ...
  it('handles nullable lastName/email/phone', async () => {
    await ensureDefaultContact(ORG, { id: 'u2', firstName: 'No', lastName: null, email: null, phone: null }, NOW)
    const data = (prisma.orgContact.create.mock.calls[0]?.[0] as { data: Record<string, unknown> }).data
  ...
  it.each([
    ['general contact', { firstName: 'A', lastName: 'B', email: 'a@b.co', phone: null }],
    ['with phone', { firstName: 'A', lastName: 'B', email: null, phone: '+10000000000' }],
...
describe('resolveMemberPermissions', () => {
  it('returns linked role permissions verbatim', async () => {
    prisma.organizationRole.findFirst.mockResolvedValue(roleRow({ permissions: ['members:read', 'org:read'] }))
    const perms = await resolveMemberPermissions({ roleId: 'rol_1', organizationId: ORG, role: 'member' })
    expect([...perms].sort()).toEqual(['members:read', 'org:read'].sort())
  })

  it('falls back to code defaults when roleId is null', async () => {
    const perms = await resolveMemberPermissions({ roleId: null, organizationId: ORG, role: 'member' })
    expect([...perms].sort()).toEqual([...defaultPermissionsForRoleName('member')].sort())
  })

  it('falls back when linked role row was deleted', async () => {
    prisma.organizationRole.findFirst.mockResolvedValue(null)
    const perms = await resolveMemberPermissions({ roleId: 'rol_gone', organizationId: ORG, role: 'owner' })
    expect(perms.size).toBeGreaterThan(0)
    expect([...perms].sort()).toEqual([...defaultPermissionsForRoleName('owner')].sort())
  })

  it('scopes role lookup to organizationId', async () => {
    prisma.organizationRole.findFirst.mockResolvedValue(roleRow({ permissions: ['org:read'] }))
    await resolveMemberPermissions({ roleId: 'rol_1', organizationId: ORG, role: 'owner' })
    expect(prisma.organizationRole.findFirst).toHaveBeenCalledWith(expect.objectContaining({ where: { id: 'rol_1', organizationId: ORG } }))
  })
  ...
  it.each(['owner', 'admin', 'member', 'billing_manager'] as const)('role %s fallback contains org:read', async (role) => {
    const perms = await resolveMemberPermissions({ roleId: null, organizationId: ORG, role })
...
describe('resolveRoleId', () => {
  it('returns id when role exists', async () => {
    prisma.organizationRole.findFirst.mockResolvedValue(roleRow({ id: 'rol_owner', name: 'owner' }))
  ...
  it('scopes query to organization', async () => {
    ...
    expect(prisma.organizationRole.findFirst).toHaveBeenCalledWith(expect.objectContaining({ where: { organizationId: ORG, name: 'owner' } }))
  })
})
...
describe('linkMembershipRole', () => {
  it('points membership at matching org role', async () => {
    prisma.organizationRole.findFirst.mockResolvedValue(roleRow({ id: 'rol_owner', name: 'owner' }))
    await linkMembershipRole({ id: 'mem_1', organizationId: ORG, role: 'owner', roleId: null }, NOW)
    expect(prisma.membership.update).toHaveBeenCalledWith({ where: { id: 'mem_1' }, data: { roleId: 'rol_owner', updatedAt: BigInt(NOW) } })
  })

  it('clears link when org has no such role', async () => {
    prisma.organizationRole.findFirst.mockResolvedValue(null)
    await linkMembershipRole({ id: 'mem_1', organizationId: ORG, role: 'ghost', roleId: 'rol_old' }, NOW)
    expect(prisma.membership.update).toHaveBeenCalledWith(expect.objectContaining({ data: { roleId: null, updatedAt: BigInt(NOW) } }))
  })

  it('does not write when already correctly linked', async () => {
    prisma.organizationRole.findFirst.mockResolvedValue(roleRow({ id: 'rol_owner', name: 'owner' }))
    await linkMembershipRole({ id: 'mem_1', organizationId: ORG, role: 'owner', roleId: 'rol_owner' }, NOW)
    expect(prisma.membership.update).not.toHaveBeenCalled()
  })

  it('no-ops when both current and target are null (ghost role, already unlinked)', async () => {
    prisma.organizationRole.findFirst.mockResolvedValue(null)
    await linkMembershipRole({ id: 'mem_1', organizationId: ORG, role: 'ghost', roleId: null }, NOW)
    expect(prisma.membership.update).not.toHaveBeenCalled()
  })

  it('uses BigInt timestamp', async () => {
    prisma.organizationRole.findFirst.mockResolvedValue(roleRow({ id: 'rol_member', name: 'member' }))
    await linkMembershipRole({ id: 'mem_1', organizationId: ORG, role: 'member', roleId: null }, NOW)
    const data = (prisma.membership.update.mock.calls[0]?.[0] as { data: { updatedAt: bigint } }).data
...
describe('assignMemberApps', () => {
  it('assigns Enterprise directory app by default', async () => {
    ...
    const call = prisma.appAssignment.upsert.mock.calls[0]?.[0] as any
  ...
  it('also assigns source app when provided', async () => {
    await assignMemberApps({ organizationId: ORG, userId: 'user_1', now: NOW, sourceAppId: 'app_couriers' })
    expect(prisma.appAssignment.upsert).toHaveBeenCalledTimes(2)
    const appIds = prisma.appAssignment.upsert.mock.calls.map((c) => (c[0] as any).create.appId)
    expect(appIds).toContain('app_couriers')
  })

  it('does not duplicate Enterprise when source is Enterprise', async () => {
    await assignMemberApps({ organizationId: ORG, userId: 'user_1', now: NOW, sourceAppId: `app_${ENTERPRISE_APP_SLUG}` })
    expect(prisma.appAssignment.upsert).toHaveBeenCalledTimes(1)
  })
  ...
  it('passes assignedBy through', async () => {
    await assignMemberApps({ organizationId: ORG, userId: 'user_1', now: NOW, assignedBy: 'actor_1' })
    const call = prisma.appAssignment.upsert.mock.calls[0]?.[0] as any
  ...
  it('does not assign source twice when sourceAppId is null', async () => {
    await assignMemberApps({ organizationId: ORG, userId: 'user_1', now: NOW, sourceAppId: null })
    expect(prisma.appAssignment.upsert).toHaveBeenCalledTimes(1)
  })

  it('creates distinct assignment ids', async () => {
    await assignMemberApps({ organizationId: ORG, userId: 'user_1', now: NOW, sourceAppId: 'app_extra' })
    const ids = prisma.appAssignment.upsert.mock.calls.map((c) => (c[0] as any).create.id)
...
describe('concurrency and determinism', () => {
  it('concurrent seedDefaultRoles calls converge: at most one create per missing role', async () => {
    ...
    const [a, b] = await Promise.all([seedDefaultRoles(ORG, NOW), seedDefaultRoles(ORG, NOW)])
  ...
  it('provisionOrgApps parallel calls each get durable rows (no double-billing)', async () => {
    const results = await Promise.all([provisionOrgApps(ORG), provisionOrgApps(ORG)])
    // First call provisions 2; second may provision 0 if findFirst now returns existing, but both should not crash.
    expect(results.length).toBe(2)
  })

  it('uses deterministic timestamps across all creates', async () => {
    await seedDefaultRoles(ORG, NOW)
    const creates = prisma.organizationRole.create.mock.calls.map((c) => (c[0] as { data: { createdAt: bigint } }).data.createdAt)
