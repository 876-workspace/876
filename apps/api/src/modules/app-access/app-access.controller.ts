...
export async function retrieveAppRoleTemplate(
  ...
  const { app_id, role_id } = validParams<{ app_id: string; role_id: string }>(
    req
  )
...
export async function retrieveOrgAppRole(
  ...
  res
    .status(200)
    .json(
      await service.retrieveOrgAppRole(org_id, app_id, role_id, principal(req))
    )
}
...
export async function listAppMemberships(
  ...
  if (query.status && !query.include_revoked)
    result.data = result.data.filter(
      (membership) => membership.status === query.status
    )
...
export async function retrieveAppMembership(
  ...
  res
    .status(200)
    .json(
      await service.retrieveAppMembership(org_id, assignment_id, principal(req))
    )
}
...
export async function updateAppMembership(
  ...
  res
    .status(200)
    .json(
      await service.updateAppMembership(
        org_id,
        assignment_id,
        body,
        principal(req)
...
export async function deleteAppMembership(
  ...
  res
    .status(200)
    .json(
      await service.deleteAppMembership(org_id, assignment_id, principal(req))
    )
}
