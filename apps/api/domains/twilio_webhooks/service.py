...
class TwilioWebhookService:
    ...
    async def apply_message_status(self, payload: dict[str, str]) -> bool:
        ...
        event = await self._repo.create_webhook_event(
            id=generate_id("webhookEvent"), provider="twilio", event_type="message.status",
            provider_sid=provider_sid, payload_hash=digest, signature_valid=True, processed_at=now,
