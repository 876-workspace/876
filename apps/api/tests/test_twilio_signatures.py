...
def test_signature_validation_includes_query_string() -> None:
    params = {"MessageSid": "SM123"}
    path = "/webhooks/twilio/messages/status?tenant=app_1"
    url = "https://public.876.example" + path
    signature = RequestValidator("auth-token").compute_signature(url, params)
    verifier = TwilioWebhookVerifier(auth_token="auth-token", webhook_base_url="https://public.876.example")

    assert verifier.validate(path=path, params=params, signature=signature)
