#!/usr/bin/env node
/**
 * Runs `prisma generate`, supplying a placeholder `BILLING_DATABASE_URL` when
 * none is set.
 *
 ...
 * 2. `prisma.config.ts` resolves `BILLING_DATABASE_URL` through Prisma's
 *    `env()`, which throws when it is unset — correct for `migrate`, which must
 *    never run against an accidental database, but `generate` only reads the
 *    schema.
 *
 * Together they mean `pnpm build` fails on any machine that has not already run
 * a migration. That is what broke the Cloudflare Workers build for
 * `876-billing-api` — the same failure `876-api` hit when the Express service
 * first reached main.
 *
...
try {
  ...
  console.error(
    'Could not resolve the Prisma CLI. Run `pnpm install` in apps/billing-api first.'
  )
  process.exit(1)
}
...
const result = spawnSync(process.execPath, [cli, 'generate'], {
  ...
  env: {
    ...process.env,
    BILLING_DATABASE_URL: process.env.BILLING_DATABASE_URL || PLACEHOLDER,
  },
})
