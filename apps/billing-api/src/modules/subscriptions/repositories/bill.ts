...
export async function billSubscription(
  ...
  const execute = async (tx: PrismaTransaction): Promise<BillResult> => {
    await tx.$queryRaw`
      SELECT id
    ...
    for (const discount of subscription.discounts) {
      if (discount.scope === 'ITEM' && discount.subscriptionItemId) {
        ...
        itemDiscounts[itemIndex] =
          itemDiscounts[itemIndex]! +
          calculateDiscount(remaining, currency, discount)
        continue
      }
      const remaining =
        subtotalAmount -
    ...
    const oneTimeLines = subscription.charges.map((charge, index) => {
      ...
      return {
        ...
        discountAmount: calculated.discountAmount + itemDiscounts[lineIndex]!,
      }
    })
    ...
    if (
      ...
      (!consolidatedInvoice ||
        (consolidatedInvoice.status === 'DRAFT') !== (invoiceMode === 'DRAFT'))
    )
      throw new Error(
