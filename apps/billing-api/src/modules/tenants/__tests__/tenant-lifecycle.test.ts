...
describe('archiveTenantRowForOrganization', () => {
  ...
  it('queries by organizationId', async () => {
    mocks.findUnique.mockResolvedValue(null)
    await archiveTenantRowForOrganization({ organizationId: 'org_query', deletedBy: null, reason: null, now: NOW })
  ...
  it('handles null deletedBy and null reason', async () => {
    mocks.findUnique.mockResolvedValue({ id: 'ten_1', deletedAt: null })
    await archiveTenantRowForOrganization({ organizationId: 'org_1', deletedBy: null, reason: null, now: NOW })
    expect(mocks.update).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ deletedBy: null, deletionReason: null }),
    }))
  })
  ...
  it('sets updatedAt to the provided now timestamp', async () => {
    ...
    mocks.findUnique.mockResolvedValue({ id: 'ten_1', deletedAt: null })
    await archiveTenantRowForOrganization({ organizationId: 'org_1', deletedBy: 'u1', reason: 'r', now: customNow })
    expect(mocks.update).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('uses tenant id in where clause', async () => {
    mocks.findUnique.mockResolvedValue({ id: 'ten_custom_123', deletedAt: null })
    await archiveTenantRowForOrganization({ organizationId: 'org_1', deletedBy: null, reason: null, now: NOW })
    expect(mocks.update).toHaveBeenCalledWith(expect.objectContaining({ where: { id: 'ten_custom_123' } }))
  })
  ...
  it('preserves first deletedAt when archiving an already-tombstoned workspace', async () => {
    ...
    await archiveTenantRowForOrganization({ organizationId: 'org_1', deletedBy: 'new_user', reason: 'new reason', now: 2_000_000 })
    expect(mocks.update).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('does not overwrite deletedBy when already tombstoned', async () => {
    mocks.findUnique.mockResolvedValue({ id: 'ten_1', deletedAt: 1_000 })
    await archiveTenantRowForOrganization({ organizationId: 'org_1', deletedBy: 'should_not_save', reason: 'should_not_save', now: NOW })
  ...
  it('returns the prisma update result', async () => {
    ...
    const result = await archiveTenantRowForOrganization({ organizationId: 'org_1', deletedBy: null, reason: null, now: NOW })
  ...
  it('handles empty string deletedBy and reason', async () => {
    mocks.findUnique.mockResolvedValue({ id: 'ten_1', deletedAt: null })
    await archiveTenantRowForOrganization({ organizationId: 'org_1', deletedBy: '', reason: '', now: NOW })
    expect(mocks.update).toHaveBeenCalledWith(expect.objectContaining({
...
describe('restoreTenantRowForOrganization', () => {
  ...
  it('queries by organizationId', async () => {
    mocks.findUnique.mockResolvedValue(null)
    await restoreTenantRowForOrganization({ organizationId: 'org_xyz', now: NOW })
    expect(mocks.findUnique).toHaveBeenCalledWith({
  ...
  it('calls findUniqueOrThrow when deletedAt is null', async () => {
    mocks.findUnique.mockResolvedValue({ id: 'ten_1', deletedAt: null })
    mocks.findUniqueOrThrow.mockResolvedValue({ id: 'ten_1', status: 'SUSPENDED', deletedAt: null })
  ...
  it('clears tombstone fields on restore', async () => {
    ...
    await restoreTenantRowForOrganization({ organizationId: 'org_1', now: NOW })
    expect(mocks.update).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ deletedAt: null, deletedBy: null, deletionReason: null }),
    }))
  })
  ...
  it('sets updatedAt to now on restore', async () => {
    ...
    await restoreTenantRowForOrganization({ organizationId: 'org_1', now: customNow })
    expect(mocks.update).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ updatedAt: customNow }),
    }))
  })
  ...
  it('returns update result on tombstoned restore', async () => {
    ...
    mocks.update.mockResolvedValue(updated)
    const result = await restoreTenantRowForOrganization({ organizationId: 'org_1', now: NOW })
    expect(result).toEqual(updated)
  })
  ...
  it('does not update when org never had workspace', async () => {
    mocks.findUnique.mockResolvedValue(null)
    await restoreTenantRowForOrganization({ organizationId: 'org_none', now: NOW })
    expect(mocks.update).not.toHaveBeenCalled()
    expect(mocks.findUniqueOrThrow).not.toHaveBeenCalled()
  })
  ...
  it('uses tenant id in where clause for restore', async () => {
    ...
    await restoreTenantRowForOrganization({ organizationId: 'org_1', now: NOW })
    expect(mocks.update).toHaveBeenCalledWith(expect.objectContaining({ where: { id: 'ten_restore_me' } }))
  })
})
