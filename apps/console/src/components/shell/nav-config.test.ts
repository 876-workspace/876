...
describe('navConfig', () => {
  ...
  it('shows admin the management entries but not security', () => {
    expect(visibleHrefs('admin')).toEqual(['/', '/users', '/orgs', '/requests', '/apps', '/widgets', '/storage', '/reports', '/settings'])
  })

  it('shows owner every entry including security', () => {
    expect(visibleHrefs('owner')).toEqual(['/', '/users', '/orgs', '/requests', '/security', '/apps', '/widgets', '/storage', '/reports', '/settings'])
  })
