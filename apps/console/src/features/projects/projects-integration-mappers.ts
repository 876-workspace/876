import type {
  ImportJobPreviewRow as ServiceImportPreviewRow,
...
const IMPORT_SOURCE_VALUES: readonly UiImportJob['source'][] = [
  'csv',
  'jira-csv',
  'jira-json',
  'trello-json',
...
function toUiImportSource(source: string): UiImportJob['source'] {
  const found = IMPORT_SOURCE_VALUES.find((entry) => entry === source)
  return found ?? 'csv'
}

export function toUiIntegrationClient(
  client: ServiceIntegrationClient
): UiIntegrationClient {
  return {
...
export function toUiWebhookEndpoint(
  endpoint: ServiceWebhookEndpoint
): UiWebhookEndpoint {
  return {
...
export function toUiDeliveryStatus(status: string): UiWebhookDelivery['status'] {
  if (status === 'delivered' || status === 'succeeded') return 'succeeded'
  if (status === 'failed') return 'failed'
  return 'pending'
}

type DeliveryWithOptionalType = ServiceWebhookDelivery & {
  eventType?: string
}

export function toUiWebhookDelivery(
  delivery: ServiceWebhookDelivery
): UiWebhookDelivery {
  const withType = delivery as DeliveryWithOptionalType
  const eventType = withType.eventType ?? delivery.eventId
  return {
...
export function toUiImportJobStatus(status: string): UiImportJob['status'] {
  ...
  if (status === 'completed' || status === 'committed' || status === 'partial')
    return 'completed'
  if (status === 'failed') return 'failed'
  return 'previewing'
}

export function toUiImportJob(job: ServiceImportJob): UiImportJob {
  return {
    object: 'projects.import-job',
    id: job.id,
    source: toUiImportSource(job.source),
...
export function toUiImportPreview(
  rows: readonly ServiceImportPreviewRow[]
): UiImportRowPreview[] {
  return rows.map((row) => ({
