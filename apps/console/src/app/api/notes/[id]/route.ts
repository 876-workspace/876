...
import { $876Member } from '@/lib/876/member'
...
export async function PATCH(request: Request, context: Ctx) {
  ...
  const result = await $876Member.notes.update({ userId: access.userId }, id, {
...
export async function DELETE(_request: Request, context: Ctx) {
  ...
  const { id } = await context.params
  const result = await $876Member.notes.delete({ userId: access.userId }, id)
  if (result.error)
    return apiError(result.error.message, {
      status: result.error.message.includes('not found') ? 404 : 502,
      code: result.error.code,
