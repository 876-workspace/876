...
export async function PATCH(
  ...
  const { data, error } = await platform.subscriptions.update(
    subscriptionId,
    body
  )
  if (error || !data)
...
export async function DELETE(
  ...
  const { subscriptionId } = await context.params
  const existing = await platform.subscriptions.retrieve(subscriptionId)
  const { data, error } = await platform.subscriptions.delete(subscriptionId)
  if (error || !data)
    return apiJson(
