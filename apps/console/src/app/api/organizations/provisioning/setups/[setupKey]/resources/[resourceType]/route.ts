...
export async function GET(_request: NextRequest, context: Context) {
  ...
  if (result.error || !result.data)
    return apiJson(
      { error: result.error?.message ?? 'Failed to list provisioning resources.' },
      { status: 400 }
    )
...
export async function POST(request: NextRequest, context: Context) {
  ...
  if (!body || typeof body !== 'object')
    return apiJson({ error: 'Invalid provisioning resource.' }, { status: 400 })

  const result = await workspace.provisioning.resources
    .forType(resourceType)
    .create(setupKey, body)
  if (result.error || !result.data)
    return apiJson(
      { error: result.error?.message ?? 'Failed to create provisioning resource.' },
      { status: 400 }
    )
