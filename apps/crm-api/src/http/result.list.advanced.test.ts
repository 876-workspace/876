...
function mockRes() {
  ...
  const json = vi.fn()
  return {
    res: { status: chainStatus, json } as unknown as import('express').Response,
    chainStatus,
    chainJson,
    json,
  }
...
describe('sendCrmList - list envelope contract', () => {
  it('sends error envelope when result is error value', () => {
    ...
    sendCrmList(
      res,
      getError('crm/category-not-found'),
  ...
  it('sends list envelope with object list metadata', () => {
    ...
    const called =
      json.mock.calls.length > 0
        ? json
        : (res as unknown as { json: typeof json }).json
  ...
  it('handles empty list as success not error', () => {
    ...
    expect(json).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ total_count: 0, data: [] }),
      })
    )
  })
  ...
  it('success list preserves ordering', () => {
    ...
    expect(json).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ data: rows }) })
    )
  })
