...
beforeEach(() => {
  ...
  repo.remove.mockResolvedValue({ object: 'request_reminder', id: 'crm_rem_1', deleted: true })
})
...
describe('reminders.service.advanced - list serialization', () => {
  it('serializes timestamps and maps object discriminator', async () => {
    ...
    repo.list.mockResolvedValue([reminderRow({ createdAt: at, updatedAt: at, remindAt })])
    const [rem] = await service.list('org_1', requestId)
  ...
  it('serializes sentAt and dismissedAt when present', async () => {
    ...
    repo.list.mockResolvedValue([reminderRow({ sentAt, dismissedAt, status: 'SENT' as const })])
    const [rem] = await service.list('org_1', requestId)
...
describe('reminders.service.advanced - create timestamp conversion', () => {
  it('converts remindAt from unix seconds to Date on write', async () => {
    ...
    await service.create('org_1', requestId, { title: 'T', remindAt: seconds, userId: 'usr_1', createdBy: 'usr_1' } as unknown as Parameters<typeof service.create>[2])
    const call = repo.create.mock.calls[0][0] as Record<string, unknown>
    expect(call.remindAt).toBeInstanceOf(Date)
    expect((call.remindAt as Date).toISOString()).toBe('2026-09-10T08:00:00.000Z')
  })

  it('preserves tenant and request binding', async () => {
    const seconds = Math.floor(Date.now()/1000)
    await service.create('org_1', requestId, { title: 'X', remindAt: seconds, userId: 'usr_2', createdBy: 'usr_1' } as unknown as Parameters<typeof service.create>[2])
    expect(repo.create).toHaveBeenCalledWith(expect.objectContaining({ tenantId, requestId, userId: 'usr_2' }))
  })

  it('does not write when context fails', async () => {
    context.requireRequestContext.mockRejectedValue({ code: 'crm/tenant-not-found' })
    await expect(service.create('org_1', requestId, { title: 'X', remindAt: 1_700_000_000, userId: 'usr_1', createdBy: 'usr_1' } as unknown as Parameters<typeof service.create>[2])).rejects.toMatchObject({ code: 'crm/tenant-not-found' })
  ...
  it('serializes created reminder with unix timestamps', async () => {
    const at = new Date('2026-08-27T00:00:00.000Z')
    const remindAt = new Date('2026-09-01T10:00:00.000Z')
    repo.create.mockResolvedValue(reminderRow({ title: 'New', createdAt: at, remindAt }))
    const result = await service.create('org_1', requestId, { title: 'New', remindAt: Math.floor(remindAt.getTime()/1000), userId: 'usr_1', createdBy: 'usr_1' } as unknown as Parameters<typeof service.create>[2])
...
describe('reminders.service.advanced - update', () => {
  it('returns null when reminder missing', async () => {
    ...
    expect(await service.update('org_1', requestId, 'missing', { title: 'x' } as unknown as Parameters<typeof service.update>[3])).toBeNull()
  ...
  it('converts remindAt on update when provided', async () => {
    ...
    await service.update('org_1', requestId, 'crm_rem_1', { remindAt: seconds } as unknown as Parameters<typeof service.update>[3])
    const call = repo.update.mock.calls[0][1] as Record<string, unknown>
    expect(call.remindAt).toBeInstanceOf(Date)
    expect((call.remindAt as Date).toISOString()).toBe('2026-09-05T12:00:00.000Z')
  })

  it('leaves remindAt untouched when not in payload', async () => {
    repo.retrieve.mockResolvedValue(reminderRow())
    await service.update('org_1', requestId, 'crm_rem_1', { title: 'x' } as unknown as Parameters<typeof service.update>[3])
    expect((repo.update.mock.calls[0][1] as Record<string, unknown>).remindAt).toBeUndefined()
  })

  it('allows status transitions', async () => {
    repo.retrieve.mockResolvedValue(reminderRow({ status: 'SCHEDULED' as const }))
    await service.update('org_1', requestId, 'crm_rem_1', { status: 'DISMISSED' } as unknown as Parameters<typeof service.update>[3])
    expect(repo.update).toHaveBeenCalledWith('crm_rem_1', expect.objectContaining({ status: 'DISMISSED' }))
  })

  it('propagates context errors', async () => {
    context.requireRequestContext.mockRejectedValue({ code: 'crm/request-not-found' })
    await expect(service.update('org_1', requestId, 'crm_rem_1', { title: 'x' } as unknown as Parameters<typeof service.update>[3])).rejects.toMatchObject({ code: 'crm/request-not-found' })
  })
})
...
describe('reminders.service.advanced - remove', () => {
  ...
  it('returns tombstone and calls repo.remove with deletedBy', async () => {
    repo.retrieve.mockResolvedValue(reminderRow())
    const res = await service.remove('org_1', requestId, 'crm_rem_1', 'usr_1')
    expect(res).toEqual({ object: 'request_reminder', id: 'crm_rem_1', deleted: true })
    expect(repo.remove).toHaveBeenCalledWith('crm_rem_1', 'usr_1')
  })

  it('propagates request-not-found', async () => {
    context.requireRequestContext.mockRejectedValue({ code: 'crm/request-not-found' })
    await expect(service.remove('org_1', requestId, 'crm_rem_1', 'usr_1')).rejects.toMatchObject({ code: 'crm/request-not-found' })
  })
