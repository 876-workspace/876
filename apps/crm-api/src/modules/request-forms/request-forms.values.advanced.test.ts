...
beforeEach(() => {
  ...
  repository.retrieve.mockResolvedValue(
    formPublished as unknown as ReturnType<typeof repository.retrieve>
  )
  repository.submissionCount.mockResolvedValue(0)
  repository.remove.mockResolvedValue({
    id: 'form_1',
    deleted: true,
  } as unknown as ReturnType<typeof repository.remove>)
  customers.list.mockResolvedValue({
    customers: [
      {
        profile: { id: 'cus_1' },
        customer: { id: 'cus_1', displayName: 'Acme' },
      },
    ],
    hasMore: false,
  } as unknown as ReturnType<typeof customers.list>)
  requests.createFromIntake.mockResolvedValue({
    id: 'req_1',
  } as unknown as ReturnType<typeof requests.createFromIntake>)
...
describe('request-forms.service - validation as values', () => {
  ...
  it('submit returns tenant-not-found value', async () => {
    ...
    const res = await service.submit('org_1', 'form_1', {
      answers: { subject: 'Hi' },
      requesterUserId: 'u1',
    } as unknown as Parameters<typeof service.submit>[2])
  ...
  it('submit returns form-not-found value', async () => {
    ...
    const res = await service.submit('org_1', 'missing', {
      answers: {},
    } as unknown as Parameters<typeof service.submit>[2])
  ...
  it('submit propagates customer registry error as value', async () => {
    ...
    customers.list.mockResolvedValue(
      err as unknown as ReturnType<typeof customers.list>
    )
    const res = await service.submit('org_1', 'form_1', {
      answers: { subject: 'Hi' },
    } as unknown as Parameters<typeof service.submit>[2])
  ...
  it('propagates request createFromIntake error as value', async () => {
    ...
    requests.createFromIntake.mockResolvedValue(
      err as unknown as ReturnType<typeof requests.createFromIntake>
    )
    const res = await service.submit('org_1', 'form_1', {
      answers: { subject: 'Hi' },
    } as unknown as Parameters<typeof service.submit>[2])
