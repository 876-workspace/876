...
beforeEach(() => {
  ...
  requireRequestContext.mockResolvedValue(
    ctx as unknown as ReturnType<typeof requireRequestContext>
  )
  repository.list.mockResolvedValue([noteRow] as unknown as ReturnType<
    typeof repository.list
  >)
  repository.create.mockResolvedValue(
    noteRow as unknown as ReturnType<typeof repository.create>
  )
  repository.retrieve.mockResolvedValue(
    noteRow as unknown as ReturnType<typeof repository.retrieve>
  )
  repository.update.mockResolvedValue(
    noteRow as unknown as ReturnType<typeof repository.update>
  )
})
...
describe('notes.service - context value propagation', () => {
  it('list propagates tenant-not-found via context', async () => {
    ...
    requireRequestContext.mockResolvedValue(
      err as unknown as ReturnType<typeof requireRequestContext>
    )
  ...
  it('create propagates request-not-found via context', async () => {
    ...
    const res = await service.create('org_1', 'req_1', {
      body: 'hi',
      authorId: 'u1',
      kind: 'NOTE',
    } as Parameters<typeof service.create>[2])
  ...
  it('update returns null when note missing', async () => {
    ...
    const res = await service.update('org_1', 'req_1', 'missing', {
      body: 'new',
      editedBy: 'u1',
