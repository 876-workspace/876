...
const { tenants, repository, finance } = vi.hoisted(() => {
  const customers = { list: vi.fn(), create: vi.fn(), retrieve: vi.fn(), update: vi.fn() }
  return {
    tenants: { retrieveByOrganization: vi.fn() },
    repository: { list: vi.fn(), retrieve: vi.fn(), create: vi.fn(), ensureMany: vi.fn(), remove: vi.fn() },
    finance: { customers },
  }
})
vi.mock('../tenants/tenants.service.js', () => tenants)
vi.mock('./customers.repository.js', () => repository)
vi.mock('@876/billing/integration', () => ({ create876BillingIntegrationClient: () => ({ customers: finance.customers }) }))
...
beforeEach(() => {
  ...
  finance.customers.create.mockResolvedValue({ data: { id: 'cus_1' }, error: null })
})
...
describe('customers.service - registry-unavailable as value', () => {
  it('retrieve propagates registry-unavailable as value not throw', async () => {
    repository.retrieve.mockResolvedValue({ id: 'prof_1', billingCustomerId: 'cus_1', tenantId: 't1', createdAt: new Date(), updatedAt: new Date(), deletedAt: null } as unknown as ReturnType<typeof repository.retrieve>)
    finance.customers.list.mockResolvedValue({ data: null as unknown as { data: [] }, error: { code: 'x', message: 'down' } })
    const res = await service.retrieve('org_1', 'prof_1')
    expect(isError(res)).toBe(true)
    expect(res).toMatchObject({ code: 'crm/registry-unavailable', httpStatus: 502 })
  })
  ...
  it('list propagates registry-unavailable as value', async () => {
    ...
    finance.customers.create.mockResolvedValue({ data: null as unknown as { id: string }, error: { code: 'x', message: 'down' } })
    const res2 = await service.create('org_1', { customerUserId: 'u1', idempotencyKey: 'k1' } as unknown as Parameters<typeof service.create>[1])
