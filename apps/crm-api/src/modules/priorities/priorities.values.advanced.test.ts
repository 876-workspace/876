...
beforeEach(() => {
  ...
  repository.list.mockResolvedValue([priorityRow] as unknown as ReturnType<
    typeof repository.list
  >)
  repository.retrieve.mockResolvedValue(
    priorityRow as unknown as ReturnType<typeof repository.retrieve>
  )
  repository.retrieveDefault.mockResolvedValue(
    priorityRow as unknown as ReturnType<typeof repository.retrieveDefault>
  )
  repository.create.mockResolvedValue(
    priorityRow as unknown as ReturnType<typeof repository.create>
  )
  repository.update.mockResolvedValue(
    priorityRow as unknown as ReturnType<typeof repository.update>
  )
  repository.remove.mockResolvedValue(
    priorityRow as unknown as ReturnType<typeof repository.remove>
  )
  repository.setDefault.mockResolvedValue({
    ...priorityRow,
    isDefault: true,
  } as unknown as ReturnType<typeof repository.setDefault>)
})
...
describe('priorities.service - value propagation advanced', () => {
  ...
  it('update returns priority-default-required when demoting default', async () => {
    ...
    const res = await service.update('org_1', 'pri_1', {
      isDefault: false,
  ...
  it('update returns null when priority not found', async () => {
    repository.retrieve.mockResolvedValue(null)
    const res = await service.update('org_1', 'missing', {
      name: 'New',
    } as Parameters<typeof service.update>[2])
    expect(res).toBeNull()
  })
