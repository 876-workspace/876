...
beforeEach(() => {
  ...
  repository.list.mockResolvedValue([categoryRow] as unknown as Awaited<
    ReturnType<typeof repository.list>
  >)
  repository.retrieve.mockResolvedValue(
    categoryRow as unknown as Awaited<ReturnType<typeof repository.retrieve>>
  )
  repository.create.mockResolvedValue(
    categoryRow as unknown as Awaited<ReturnType<typeof repository.create>>
  )
  repository.update.mockResolvedValue(
    categoryRow as unknown as Awaited<ReturnType<typeof repository.update>>
  )
  repository.remove.mockResolvedValue({
    id: 'cat_1',
    deleted: true,
  } as unknown as ReturnType<typeof repository.remove> extends Promise<infer T>
    ? T
    : never)
})
...
describe('categories.service - tenant value propagation', () => {
  ...
  it('list success maps to serialized categories', async () => {
    ...
    expect((res as unknown as Array<Record<string, unknown>>)[0].object).toBe(
      'request_category'
    )
  })
  ...
  it('remove returns tenant error without touching repository', async () => {
    tenants.retrieveByOrganization.mockResolvedValue(null)
    const res = await service.remove('org_1', 'cat_1', {
      deletedBy: 'u1',
    } as Parameters<typeof service.remove>[2])
    expect(isError(res)).toBe(true)
