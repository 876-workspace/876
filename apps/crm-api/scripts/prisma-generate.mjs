#!/usr/bin/env node
/**
 * Runs `prisma generate`, supplying a placeholder `CRM_DATABASE_URL` when none is
 * set.
 *
 ...
 * 2. `prisma.config.ts` resolves `CRM_DATABASE_URL` through Prisma's `env()`, which
 ...
 * `PrismaConfigEnvError: Cannot resolve environment variable: CRM_DATABASE_URL`
...
try {
  ...
  console.error(
    'Could not resolve the Prisma CLI. Run `pnpm install` in apps/crm-api first.'
  )
  process.exit(1)
}
...
const result = spawnSync(process.execPath, [cli, 'generate'], {
  ...
  env: {
    ...process.env,
    CRM_DATABASE_URL: process.env.CRM_DATABASE_URL || PLACEHOLDER,
  },
})
