...
import { getWidgets } from '@/lib/clients/widgets'
...
export async function GET() {
  ...
  const result = await getWidgets().collections.list({ userId: access.userId })
  if (result.error)
...
export async function POST(request: Request) {
  ...
  const result = await getWidgets().collections.create(
