...
async function route(
  ...
  if (request.method !== 'GET') {
    ...
    if (!billingContext?.permissions.includes('roles:write'))
      return apiError('You do not have permission to manage roles.', {
        status: 403,
      })
  }

  const { path = [] } = await context.params
  return proxyBillingResourceRequest(request, 'roles', path)
}
