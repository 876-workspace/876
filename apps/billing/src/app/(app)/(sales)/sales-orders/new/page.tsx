  ...
  return (
    <Page>
      <PageHeader>
        ...
        <PageDescription>Create a draft commercial commitment.</PageDescription>
      </PageHeader>
      <DocumentCreateForm
        kind="sales-order"
        defaultCurrency={context.tenant.defaultCurrency}
        returnUrl="/sales-orders"
        items={[
          ...items.map((item) => ({
            value: `item:${item.id}`,
            label: item.name,
          ...
          ...prices.map((price) => ({
            ...
            label: `${price.item?.name ?? price.nickname ?? 'Catalog price'} · ${formatPriceCadence(price)}`,
            itemId: price.itemId,
            priceId: price.id,
            defaultAmount:
