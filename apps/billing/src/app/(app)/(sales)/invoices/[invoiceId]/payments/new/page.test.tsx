...
vi.mock('@/features/payments/components/payment-form', () => ({
  PaymentForm: ({
    prefill,
    returnHref,
  }: {
...
describe('NewInvoicePaymentPage', () => {
  beforeEach(() => {
    ...
    mocks.requirePagePermission.mockResolvedValue({
      tenant: { id: 'tenant_123', defaultCurrency: 'JMD' },
    })
  ...
  it('refuses an unauthorized principal before loading billing data', async () => {
    mocks.requirePagePermission.mockRejectedValue(new Error('REDIRECT:/no-access?reason=permission'))

    await expect(
      NewInvoicePaymentPage({ params: Promise.resolve({ invoiceId: invoice.id }) })
    ).rejects.toThrow('REDIRECT:/no-access?reason=permission')
