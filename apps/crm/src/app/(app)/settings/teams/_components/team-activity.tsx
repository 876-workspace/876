...
/**
 * The timeline reports only what the team record actually carries — its
 * creation and its most recent change. Per-change history is not recorded yet,
 * so nothing here is inferred beyond those two timestamps.
 */
function buildEvents(team: TeamRow): ActivityEvent[] {
    ...
    events.push(
      team.status === 'ARCHIVED'
        ? {
            ...
            tone: 'amber',
            title: 'Team archived',
            detail: 'Archived teams stop receiving new request assignments.',
            at: team.updatedAt,
          }
        : {
            id: 'updated',
...
export function TeamActivity({ team }: { team: TeamRow }) {
  const events = buildEvents(team)
