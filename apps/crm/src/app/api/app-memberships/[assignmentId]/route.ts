...
function invalidBody() {
  return Response.json(
    {
      data: null,
      error: { code: 'crm/invalid-request', message: 'Invalid request body.' },
    },
    { status: 400 }
  )
}
...
export async function PATCH(request: NextRequest, route: RouteContext) {
  ...
  if (!input.success) return invalidBody()
  const { assignmentId } = await route.params
  const workspace = await getWorkspace()
  const result = await workspace.appMemberships.update(
    context.orgId,
    assignmentId,
    input.data
  )
...
export async function DELETE(_request: NextRequest, route: RouteContext) {
  ...
  const { assignmentId } = await route.params
  const workspace = await getWorkspace()
  const result = await workspace.appMemberships.delete(
    context.orgId,
    assignmentId
  )
  return Response.json(result, { status: result.error ? 502 : 200 })
