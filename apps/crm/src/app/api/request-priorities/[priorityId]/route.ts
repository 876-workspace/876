...
function statusFor(code: string | undefined) {
  ...
  if (code === 'crm/priority-default-required' || code === 'crm/priority-in-use') return 409
  return 400
}
...
export async function PATCH(request: NextRequest, route: Context) {
  ...
  const result = await crm.requestPriorities.update(context.orgId, priorityId, input as never)
...
export async function DELETE(_request: NextRequest, route: Context) {
  ...
  if (!context) return unauthorized()
  const { priorityId } = await route.params
  const result = await crm.requestPriorities.delete(context.orgId, priorityId, { deletedBy: context.userId })
  return Response.json(result, { status: result.error ? statusFor(result.error.code) : 200 })
}
