...
function statusFor(code: string | undefined) { return code === 'crm/request-not-found' ? 404 : 400 }
...
export async function PATCH(request: NextRequest, route: Context) {
  ...
  const result = await crm.requests.update(context.orgId, requestId, input as never)
  return Response.json(result, { status: result.error ? statusFor(result.error.code) : 200 })
}
...
export async function DELETE(request: NextRequest, route: Context) {
  ...
  const result = await crm.requests.delete(context.orgId, requestId, { deletedBy: context.userId, reason: input.reason ?? null })
  return Response.json(result, { status: result.error ? statusFor(result.error.code) : 200 })
}
