...
vi.mock('@/lib/clients/crm', () => ({
  crm: { requestTasks: { update: mocks.update, delete: mocks.remove } },
}))

const URL_ = 'http://localhost/api/requests/crm_req_1042/tasks/crm_task_7'
const ROUTE = {
  params: Promise.resolve({ requestId: 'crm_req_1042', taskId: 'crm_task_7' }),
}
  ...
  describe('PATCH', () => {
    it('attributes completion to the signed-in member', async () => {
      ...
      expect(mocks.update).toHaveBeenCalledWith(
        ...
        'crm_req_1042',
        'crm_task_7',
        { status: 'DONE', completedBy: 'user_session_123' }
      )
    })
    it('ignores a body-supplied completedBy value', async () => {
      await PATCH(
        patchRequest({ status: 'DONE', completedBy: 'user_attacker_999' }),
        ROUTE
      )
      expect(mocks.update).toHaveBeenCalledWith(
        'org_island_123',
    ...
    it('trims a supplied title', async () => {
      ...
      expect(mocks.update).toHaveBeenCalledWith(
        ...
        'crm_req_1042',
        'crm_task_7',
        { title: 'Chase the courier', completedBy: 'user_session_123' }
      )
    })
    ...
    it('maps a missing task to 404', async () => {
      ...
      const response = await PATCH(patchRequest({ status: 'OPEN' }), ROUTE)
