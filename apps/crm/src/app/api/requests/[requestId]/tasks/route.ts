...
export async function POST(request: NextRequest, route: Context) {
  const context = await getCrmApiContext(); if (!context) return unauthorized()
  const { requestId } = await route.params
  const input = (await request.json().catch(() => null)) as Partial<Omit<CrmRequestTaskCreateInput, 'createdBy'>> | null
  const title = input?.title?.trim()
  if (!title) return Response.json({ data: null, error: { code: 'crm/invalid-body', message: 'A task needs a title.' } }, { status: 400 })
  const result = await crm.requestTasks.create(context.orgId, requestId, { ...input, title, createdBy: context.userId })
