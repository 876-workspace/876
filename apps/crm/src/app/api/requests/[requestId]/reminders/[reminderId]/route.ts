...
function statusFor(code: string | undefined) {
  if (code === 'crm/request-not-found' || code === 'crm/reminder-not-found')
    return 404
  return 400
}
...
export async function PATCH(request: NextRequest, route: Context) {
  ...
  const input = (await request
    .json()
    .catch(() => null)) as CrmRequestReminderUpdateInput | null
  ...
  const result = await crm.requestReminders.update(
    context.orgId,
    requestId,
    reminderId,
...
export async function DELETE(_request: NextRequest, route: Context) {
  ...
  if (!context) return unauthorized()
  const { requestId, reminderId } = await route.params
  const result = await crm.requestReminders.delete(
    context.orgId,
    requestId,
    reminderId,
