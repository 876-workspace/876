...
function statusFor(code: string | undefined) { if (code === 'crm/request-not-found' || code === 'crm/request-note-not-found') return 404; if (code === 'crm/description-note-immutable') return 409; return 400 }
...
export async function PATCH(request: NextRequest, route: Context) {
  ...
  const result = await crm.requestNotes.update(context.orgId, requestId, noteId, { body: input.body.trim(), editedBy: context.userId })
...
export async function DELETE(_request: NextRequest, route: Context) {
  const context = await getCrmApiContext(); if (!context) return unauthorized()
  const { requestId, noteId } = await route.params
  const result = await crm.requestNotes.delete(context.orgId, requestId, noteId, { deletedBy: context.userId })
  return Response.json(result, { status: result.error ? statusFor(result.error.code) : 200 })
}
