...
function statusFor(code: string | undefined) { if (code === 'crm/request-not-found' || code === 'crm/event-not-found') return 404; return 400 }
...
export async function GET(_request: NextRequest, route: Context) {
  ...
  const result = await crm.requestEvents.retrieve(context.orgId, requestId, eventId)
...
export async function PATCH(request: NextRequest, route: Context) {
  ...
  const result = await crm.requestEvents.update(context.orgId, requestId, eventId, input)
  return Response.json(result, { status: result.error ? statusFor(result.error.code) : 200 })
}
...
export async function DELETE(_request: NextRequest, route: Context) {
  const context = await getCrmApiContext(); if (!context) return unauthorized()
  const { requestId, eventId } = await route.params
  const result = await crm.requestEvents.delete(context.orgId, requestId, eventId, { deletedBy: context.userId })
  return Response.json(result, { status: result.error ? statusFor(result.error.code) : 200 })
}
