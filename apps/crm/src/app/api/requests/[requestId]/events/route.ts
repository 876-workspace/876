...
function statusFor(code: string | undefined) { if (code === 'crm/request-not-found' || code === 'crm/calendar-not-found') return 404; return 400 }
...
export async function POST(request: NextRequest, route: Context) {
  const context = await getCrmApiContext(); if (!context) return unauthorized()
  const { requestId } = await route.params
  const input = (await request.json().catch(() => null)) as Partial<Omit<CrmRequestEventCreateInput, 'createdBy'>> | null
  const title = input?.title?.trim()
  if (!title) return Response.json({ data: null, error: { code: 'crm/invalid-body', message: 'An event needs a title.' } }, { status: 400 })
  const result = await crm.requestEvents.create(context.orgId, requestId, { ...input, title, createdBy: context.userId } as CrmRequestEventCreateInput)
