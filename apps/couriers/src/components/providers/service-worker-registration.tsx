...
export function ServiceWorkerRegistration() {
  useEffect(() => {
    ...
    if ('serviceWorker' in navigator && shouldRegister) {
      ...
      // The worker caches `/_next/static/` CacheFirst for a year. Turbopack
      // reuses those dev chunk paths across compiles, so an edited component
      // keeps hydrating from an earlier build — the sidebar renders correctly
      // from the server, then flips back to the stale nav on hydration.
      void Promise.all([
