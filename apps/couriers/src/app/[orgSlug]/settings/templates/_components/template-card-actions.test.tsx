...
describe('TemplateCardActions', () => {
  ...
  it('sets the template as default with the org slug and id then refreshes', async () => {
    ...
    expect(mocks.setDefault).toHaveBeenCalledWith('island-logistics', 'dtpl_1')
  ...
  it('deletes with the org slug and id after confirmation then refreshes', async () => {
    const user = userEvent.setup()
    renderActions()
    ...
    expect(mocks.remove).toHaveBeenCalledWith('island-logistics', 'dtpl_1')
    expect(mocks.refresh).toHaveBeenCalledTimes(1)
    await waitFor(() =>
