...
export function OnboardingForm({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (submitting || (!existingOrgName && !name.trim())) return
      ...
      >
        <div>
          <h1 className="876-page-title">
            {existingOrgName ? 'Add 876 Commerce' : 'Set up your organization'}
          </h1>
          <p className="text-muted-foreground mt-1 text-sm">
            {existingOrgName
              ? `Turn on 876 Commerce for ${existingOrgName}.`
              : 'Name the organization that will own this Commerce workspace.'}
          </p>
        </div>
