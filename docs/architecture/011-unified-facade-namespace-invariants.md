...
| ~~Console `$876.subscriptions.admin`~~ — reviewed and found **not** a collision                                                                     | `@876/admin`'s top-level `subscriptions` hits `/billing/subscriptions` (platform billing records); the org→app entitlement is separately `workspace.apps.entitlements` on the workspace control plane. The architecture already keeps them distinct. |
