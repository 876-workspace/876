...
| ---------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------- |
| **Core identity / platform** | `@876/api` (FastAPI) + its Postgres | users, orgs, memberships, org-roles, features, auth, oauth, geo, addresses, `audit_events`, vendors/customers/locations, future plans/pricing, `organization_app_access` | `$876` (`@876/sdk` / `@876/admin`)              |
...
| **Shared platform services** | each its own bounded context + DB   | future: ticketing/disputes, commerce/orders, messaging                                                                                                                   | `$876.<resource>.<verb>()`, auth-tiered         |

**Concrete instance: org → platform-app provisioning.** `organization_app_access` is the entitlement table controlling which orgs can access which 876 platform apps. It lives in the core identity API — not any single app's datastore — because it is cross-cutting: the couriers app reads it to gate dashboard access, Console reads and writes it to provision/block orgs, and future apps follow the same pattern. The pattern: API owns the table + `AdminDep` endpoints under `domains/organizations/`; apps gate via `$876.orgs.appAccess.retrieveBySlug` in their `ManageContext`; new-org sign-up auto-provisions in `auth/complete` when `source=register` is set; Console provides provision/block controls via its own route handlers.

### How to place something

1. **Is it identity or platform-foundational** (who a user/org is, what they can
   sign into, plans/pricing, features, auth)? → **Core identity API.**
...
- Every app-local datastore follows the same `<resource>.<verb>()` naming vocabulary as `$876`, in two layers: the `prisma` singleton (`@/lib/db`) and the `service.<resource>.<verb>()` layer (`@/lib/service`, the only caller allowed to query `prisma`) — see "App-local datastore layering" in `.grok/rules/sdk-conventions.md`.
...
- **Field visibility is an API serializer concern**, never client-side filtering —
  the same rule as `.grok/rules/sdk-conventions.md`.
...
- [ ] If gating an app to specific orgs: use the `organization_app_access` provisioning pattern (not `OrgFeature`); gate in the app's `ManageContext` / private layout via `$876.orgs.appAccess.retrieveBySlug`; auto-provision at `auth/complete` with `source=` param.
