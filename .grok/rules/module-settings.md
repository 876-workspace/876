...
one. Companion to `.grok/rules/platform-services.md` (three-bucket placement),
`.grok/rules/sdk-conventions.md` (client surface), and
`.grok/rules/app-layout.md` (the settings pages themselves).
...
**A module is not a feature flag.** A feature flag (`.grok/rules/feature-flags.md`)
...
By decision step #2 of `.grok/rules/platform-services.md`, module state and
...
- Reads and writes go through the app's `service.<resource>.<verb>()` layer — the
  only caller allowed to touch `prisma` (`.grok/rules/sdk-conventions.md`).
...
`ResourceToolbar`'s `DropdownAction.icon` already uses (`.grok/rules/app-layout.md`).
