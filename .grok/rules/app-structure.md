...
Companion to `.grok/rules/app-layout.md` (what a page _looks_ like),
`.grok/rules/sdk-conventions.md` (`src/lib/` layering), and
`.grok/rules/types.md` (where types live).
...
- A route directory should contain **only** route files — `page.tsx`,
  `layout.tsx`, `loading.tsx`, `error.tsx`, `not-found.tsx`, `route.ts` — plus
  `_components/` and, where used, `_data.ts` / `_lib/`.
...
app needs should be promoted to `packages/ui`. If `patterns/` grows past ~10
files, that is the signal that promotions are overdue, not that the folder
needs subfolders.
...
Follow `.grok/rules/types.md`. Within this structure:
...
`.grok/rules/performance-bundle-size.md` §2.1.
...
| ------------------ | --------------------- | ---------------------------------------------------------------- |
| `876.ts` or `876/` | every app             | the `$876` singleton (see `.grok/rules/sdk-conventions.md`)      |
...
"somewhere in components for now". See `.grok/rules/new-app-guide.md` for the
