...
`.agents/rules/app-structure.md`: at 300+ endpoints it produces a `services/`
...
- **Schemas are named `camelCase` ending in `Schema`**; inferred types are
  `PascalCase`. Same as `.agents/rules/types.md`.
...
- **Every serialized resource carries a literal `object` discriminator**
  (`z.literal('user')`), per `.agents/rules/stripe-api-pattern.md`.
...
The tier model from `.agents/rules/platform-services.md` is implemented as
...
- **The database is snake_case; the client is camelCase.** Every model carries
  ...
  or column is forbidden by `.agents/rules/naming.md` — the map attribute is how
...
- **Soft deletes per `.agents/rules/deletions.md`.** End-user reads filter
...
Follows `.agents/rules/testing.md`. Service-specific additions:
...
- Do not rename a database table, column, env var, or error code — they are
  contracts (`.agents/rules/naming.md`).
