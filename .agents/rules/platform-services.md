...
**Concrete instance: org → platform-app provisioning.** `subscriptions` (the `Subscription` model — the table was renamed from `organization_app_access` by migration) is the entitlement table controlling which orgs can access which 876 platform apps. It lives in the core identity API — not any single app's datastore — because it is cross-cutting: the couriers app reads it to gate dashboard access, Console reads and writes it to provision/block orgs, and future apps follow the same pattern. New-org provisioning runs through `provisionOrganization` in `apps/api/src/services/provisioning.ts`: every org receives the `876-enterprise` default entitlement, and the validated source app is also provisioned. `876-billing` is an explicit application entitlement; it is not a default. Apps gate through `platform.subscriptions.retrieve` in their management context, and Console provides provision/block controls through its route handlers. Product access is independent of any app-local tenant row.
