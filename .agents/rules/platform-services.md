...
| **App-local operational**    | the app itself, its own datastore   | Console users (access grants), Console roles, staff notes, Console settings (`import { records } from '@/lib/records'`)                                                                                                                                                      | imported directly, server-only, inside that app                 |
...
- Every app-local datastore follows the same `<resource>.<verb>()` naming
  ...
  (`@/lib/db`) and the `records.<resource>.<verb>()` layer (`@/lib/records`, the
