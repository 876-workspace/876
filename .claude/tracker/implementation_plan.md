...
- Treat `@876/crm` as the canonical owner of CRM wire contracts; the CRM API may consume a contracts-only package entrypoint rather than redefining public resource/input types.
...
- [ ] Add a contracts-only `@876/crm/contracts` entrypoint.
- [ ] Make CRM API request contracts consume the canonical CRM package contracts.
- [ ] Update `@876/crm-api` dependencies for the contracts boundary.
- [ ] Normalize CRM `/api/auth/switch-org` to shared API envelope helpers and the app auth schema.
- [ ] Add or update tests/contract checks where existing test structure allows a safe remote edit.
- [ ] Add anti-slop/reuse-first repository guidance and mirror it into `.agents/rules/`.
- [ ] Audit touched code for deprecated compatibility residue; remove only items proven unused/in-repo and safe.
- [ ] Self-review branch diff for duplication, `eslint-disable`, `as any`, dead compatibility aliases, and accidental scope growth.
