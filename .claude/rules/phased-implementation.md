...
- `.claude/rules/git.md` — the integration-branch mechanics (branching, merge
  subjects, stacked phases, the final `main` PR).
- `.claude/rules/cli.md` — which model or CLI handles which class of work, and
  the briefing format.
- `.claude/rules/implementation-tracker.md` — the local, gitignored tracker.
...
problem, later).
**Supersedes:** none.
**Refs:** PR #96, `.claude/rules/module-settings.md`.
```
...
| **Rule file** (`.claude/rules/`)       | Standing behaviour every future change must follow.                    | Until the rule changes            |
| **PR description**                     | The reviewable narrative for this change set.                          | Tied to the diff                  |
| **Local tracker** (`.claude/tracker/`) | Scratch checklist for the current session.                             | Disposable, gitignored            |
...
- **The workspace conventions**, restated: the team, the project taxonomy, the
  ...
  conventions in memory or `.claude/rules/` for the current values — the
...
4. **Write a brief per phase** into `.claude/briefs/<tool>/<date>-<task>.md` and
