...
Console is an internal operator surface and may expose more diagnostic context directly in the UI:

- show the stable error code by default (`showCode`);
- show safe descriptions/params when useful;
- link or correlate to request IDs/Sentry where available;
- never expose secrets, raw SQL, provider credentials, stack traces, or sensitive payloads.
...
- provider access denial -> registered authorization/provider value;
- provider unavailable -> registered dependency value plus internal logging;
- unexpected driver/runtime crash -> capture/throw as an unexpected exception, then contain it at the nearest safe UI/server boundary.

## Future app checklist

Before a new app or resource family is complete:
...
When reviewing old code, classify the failure first. Preserve throws for true exceptions, convert expected application outcomes to values, and keep recoverable failures inside the live UI instead of escalating them to the framework error page.
