...
export async function getCrm() {
  ...
  return create876CrmSessionClient({ accessToken: session.accessToken })
}
```
...
`create()` describes a domain operation, not a database insert. Cross-service side effects belong behind the backend API that owns the business operation.

Bad:

```ts
const file = await storage.files.create(...)
await crm.requests.update(...)
await work.tasks.create(...)
```

when those calls collectively implement one CRM operation.

Correct: expose one CRM capability and let the CRM backend coordinate Storage/Work internally through first-party service clients.

A page may independently read several domains for display; that is not the same as implementing a distributed business transaction in the host.
...
An application-owned Prisma datastore remains separate from remote 876 service clients. The approved two-layer pattern remains:

1. request-safe `prisma`/database runtime in `src/lib/db`;
2. `service.<resource>.<verb>()` in `src/lib/service`, which is the only layer allowed to query that app database.

The same verb vocabulary applies. Do not confuse an app-local datastore `service` namespace with the cross-service **service principal**; context and import path make the distinction clear.
...
7. If multiple hosts render the same product UI, reuse `@876/<product>-ui` rather than duplicating the screen.

Do not update a global product switch or global ownership manifest; those are artifacts of the retired mega-facade architecture.

## Migration policy

`@876/client`, the generic `@876/sdk`, and generic `@876/admin` are legacy migration surfaces. Do not add new production call sites to them. Move existing responsibilities to `@876/account`, `@876/workspace`, `@876/platform`, or the owning product package and remove the legacy packages once no production consumers remain.

Do not add compatibility aliases for migrated product resources. The repository should converge on one architecture, not maintain both indefinitely.
