...
New-org provisioning runs inside `services/auth.py: register_business` via `provision_org_apps`. Every org receives `DEFAULT_ORG_APP_SLUGS` = `876-enterprise` (the directory where an org manages itself) + `876-billing` (its financial plane — invoices, payment methods, the customer registry) regardless of sign-up origin. In addition, the app the signup came through is provisioned; its identity is taken from `request.state.app_id` (the API key the request authenticated with), never from a client-supplied field — an app cannot claim to be a different app. Provisioning is idempotent: an existing subscription is left in place and a missing app row logs `provisioning.default_app_missing` and is skipped rather than failing the signup. See `docs/org-provisioning.md` for operational details.
