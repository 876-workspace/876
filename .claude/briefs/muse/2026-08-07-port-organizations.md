# Brief — port the `organizations` module to Express
...
Port `domains/organizations/` (28 routes, 4,508 lines across three routers) to
`src/modules/organizations/` (Express 5, TypeScript), behaviour-identical.

Three FastAPI routers, all mounted on `protected_router` in `api/v1.py`:

- `domains/organizations/router.py` — the core organization resource
- `domains/organizations/structure.py` — org structure (locations, contacts, …)
- `domains/organizations/access.py` — org access / app subscriptions
...
`services/organization-bootstrap.ts`, `services/provisioning.ts`,
`services/identity-sync.ts` and `services/billing-customer-sync.ts` are **already
ported** — call them, do not reimplement their logic in the module.

## The source to port

- `domains/organizations/router.py`, `structure.py`, `access.py`, `schemas.py`,
  `docs.py`
- whichever `db/repositories/*.py` they touch — find them by reading the routers
- the matching models under `db/models/`

## Files you may create

Under `src/modules/organizations/` only. `organizations.docs.ts` **already
exists** — import the prose from it; do not rewrite it and do not put prose in
the route files.

Export one router factory per group from `index.ts`
(`registerOrganizationRoutes`, `registerOrgStructureRoutes`,
`registerOrgAccessRoutes` — mirroring how `src/modules/directory/index.ts` does
it), and name them in your report.

## Files you may edit

Only files under `src/modules/organizations/`. **Do not edit
`src/http/routes.ts`** — the orchestrating agent mounts every module in one edit
at the end of the batch, and concurrent edits to it collide.
...
Org→app entitlements live in the `subscriptions` table (`Subscription` model —
renamed from `organization_app_access`). It is **not** `OrgFeature`. The batch
endpoint (`/app-access/batch?organization_ids=…`) exists to avoid an N+1 in list
views; keep it, and keep the `app` relation included so `app.slug` is available
