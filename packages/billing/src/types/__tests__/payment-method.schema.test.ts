...
describe('PaymentMethodSchema', () => {
  ...
  it('rejects credential field (never client-safe)', () => {
    const withCredential = { ...validMethod(), credential: { sealedValue: 'la1:secret' } } as any
    const result = PaymentMethodSchema.safeParse(withCredential)
    expect(result.success).toBe(false)
  })

  it('rejects unknown type', () => {
    expect(PaymentMethodSchema.safeParse(validMethod({ type: 'CRYPTO' } as any)).success).toBe(false)
  })

  it('rejects invalid status', () => {
    expect(PaymentMethodSchema.safeParse(validMethod({ status: 'UNKNOWN' } as any)).success).toBe(false)
  })

  it('requires isDefault boolean', () => {
    expect(PaymentMethodSchema.safeParse(validMethod({ isDefault: undefined } as any)).success).toBe(false)
  })

  it('rejects unknown fields (strict)', () => {
    expect(PaymentMethodSchema.safeParse({ ...validMethod(), unknown: 'x' } as any).success).toBe(false)
  })

  it('validates each type enum', () => {
    for (const type of ['CARD', 'BANK_ACCOUNT', 'WALLET', 'MANUAL'] as const) {
      expect(PaymentMethodSchema.safeParse(validMethod({ type })).success).toBe(true)
    }
  })

  it('validates each status enum', () => {
    for (const status of ['PENDING', 'ACTIVE', 'REQUIRES_ACTION', 'EXPIRED', 'DETACHED', 'FAILED'] as const) {
      expect(PaymentMethodSchema.safeParse(validMethod({ status })).success).toBe(true)
    }
  })
})
...
describe('DeletedPaymentMethodSchema', () => {
  it('accepts deleted tombstone', () => {
    expect(DeletedPaymentMethodSchema.safeParse({ object: 'payment_method', id: 'pm_1', deleted: true }).success).toBe(true)
  })
  it('rejects missing deleted flag', () => {
    expect(DeletedPaymentMethodSchema.safeParse({ object: 'payment_method', id: 'pm_1' } as any).success).toBe(false)
  })
})
