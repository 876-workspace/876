...
/**
 * Request validation — the one place a service parses raw input.
 *
 * Validation happens here and nowhere else: a controller reads `req.valid.body`
 * and never re-parses raw input (.claude/rules/express-api.md). Every 876 Express
 * service shares this single implementation so the three copies that had already
 * drifted (a `WeakMap` variant that dropped the typed helper) cannot reappear.
 */
