...
export interface PaymentReceivedOption {
  value: string
  label: string
}

export interface PaymentReceivedAccountOption extends PaymentReceivedOption {
  currency: string
}

export interface PaymentReceivedInvoiceOption {
  id: string
  customerId: string
  number: string
  currency: string
...
export interface PaymentReceivedInitial {
  id: string
  number: string
  customerId: string
...
export interface PaymentReceivedPrefill {
  customerId?: string
  invoiceId?: string
}

export interface PaymentReceivedSubmitParams {
  customerId: string
  paymentModeId: string
  depositAccountId: string
  amount: string
...
export interface PaymentReceivedActionResult {
  error: string | null
}
...
export interface PaymentReceivedFormProps {
  ...
  onSubmit: (
    params: PaymentReceivedSubmitParams
  ) => Promise<PaymentReceivedActionResult>
  onDelete?: () => Promise<PaymentReceivedActionResult>
  onCancel: () => void
}

/** Shared Payments Received editor used by Billing and Invoice hosts. */
export function PaymentReceivedForm({
  ...
  function remove() {
    if (!onDelete || !window.confirm('Delete and reverse this payment?')) return
    setError(null)
    startTransition(async () => {
...
function formatMoney(
  ...
  const value = formatMinorAmountInput(amount, decimalPlaces)
