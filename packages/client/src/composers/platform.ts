...
export function createPlatformServerClient(options: PlatformServerClientOptions) {
  const services = createServiceClients(options)
  if (services.platformAdmin) {
    return createCoreSurface({ platform: services.platform, admin: services.platformAdmin })
  }
