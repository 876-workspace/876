...
describe('security - error messages never leak internals', () => {
  ...
  it('toAppError strips all server-only fields', () => {
    ...
    >) {
      ...
      expect(
        (client as unknown as Record<string, unknown>).httpStatus
      ).toBeUndefined()
      expect(
        (client as unknown as Record<string, unknown>).description
      ).toBeUndefined()
      expect(
        (client as unknown as Record<string, unknown>).param
      ).toBeUndefined()
  ...
  it('isError guard rejects forged error lacking httpStatus number', () => {
    expect(
      isError({ code: 'crm/team-not-found', message: 'Team not found.' })
    ).toBe(false)
    expect(
      isError({
