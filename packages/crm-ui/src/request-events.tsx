...
import type { CreateRequestEventInput, RequestEvent } from '@876/crm'
...
export type RequestEventsSectionProps = {
  events: readonly RequestEvent[]
  onCreate: (
    input: RequestEventCreateInput
  ) => Promise<RequestEventMutationResult>
  onDelete: (eventId: string) => Promise<RequestEventMutationResult>
  onChanged?: () => void
}
...
function formatEvent(event: RequestEvent) {
  if (event.allDay) {
...
/**
 * Shared CRM schedule surface. The host supplies mutations, so standalone CRM,
 * Console operator views and future integrations can render one interface
 * without sharing credentials or browser transport.
 */
export function RequestEventsSection({
  ...
  async function createEvent(event: React.FormEvent) {
    ...
    const startAt = unix(draft.start)
    const endAt = unix(draft.end)
    let input: RequestEventCreateInput | null = null

    if (draft.allDay && draft.startDate && draft.endDate)
      input = {
        title,
        allDay: true,
    ...
    else if (!draft.allDay && startAt != null && endAt != null)
      input = { title, allDay: false, startAt, endAt, timeZone }
  ...
  async function deleteEvent(eventId: string) {
    setError(null)
    setBusyId(eventId)
    const result = await onDelete(eventId)
    setBusyId(null)
