...
export function createCalendarSubscriptionsResource(runtime: WorkRuntime) {
  return {
    list(organizationId: string, calendarId: string) {
      return workRequest(
        runtime,
        { method: 'GET', path: root(organizationId, calendarId) },
        workCalendarSubscriptionListSchema
      )
    },
    create(
      ...
      return workRequest(
        runtime,
        { method: 'POST', path: root(organizationId, calendarId), body: input },
        workCalendarSubscriptionSchema
      )
    },
    update(
      ...
      return workRequest(
        ...
        {
          method: 'PATCH',
          path: `${root(organizationId, calendarId)}/${encodeURIComponent(subscriptionId)}`,
          body: input,
        },
        workCalendarSubscriptionSchema
      )
