...
export function createCalendarsResource(runtime: WorkRuntime) {
  return {
    list(organizationId: string, filter: WorkCalendarListFilter = {}) {
      return workRequest(runtime, { method: 'GET', path: listPath(organizationId, filter) }, workCalendarListSchema)
    },
    retrieve(organizationId: string, calendarId: string) {
      return workRequest(runtime, { method: 'GET', path: `${root(organizationId)}/${encodeURIComponent(calendarId)}` }, workCalendarSchema)
    },
    create(organizationId: string, input: CreateWorkCalendarInput) {
      return workRequest(runtime, { method: 'POST', path: root(organizationId), body: input }, workCalendarSchema)
    },
    update(organizationId: string, calendarId: string, input: UpdateWorkCalendarInput) {
      return workRequest(runtime, { method: 'PATCH', path: `${root(organizationId)}/${encodeURIComponent(calendarId)}`, body: input }, workCalendarSchema)
    },
    delete(organizationId: string, calendarId: string, deletedBy: string) {
      return workRequest(runtime, { method: 'DELETE', path: `${root(organizationId)}/${encodeURIComponent(calendarId)}`, body: { deletedBy } }, z.object({ object: z.literal('calendar'), id: z.string(), deleted: z.literal(true) }))
    },
  }
}
